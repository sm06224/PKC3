...
6. 🔴 **実装担当が「未検証の穴」「既知の限界」と自己申告したものは、依頼文の要求と
   突き合わせる**(2026-09-26、#1038 段 G)。⚠ 1 巡目の検証は、実装担当が申告した
   「一覧で別々のフォルダのノートを選んで移すと、一部が外れうる」を**「コード変更を
   伴わない既知の限界」として通した** ── 依頼文は「移す…は選んだ全部に効く」と
   書いていたので、それは限界ではなく**欠陥**だった(後の段が読み直して見つけた)。
