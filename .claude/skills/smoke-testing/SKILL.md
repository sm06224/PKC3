...
1. **単独で再現するか** ── `npx playwright test … <grep>` を 3〜5 回。
   単独で緑・全量で赤なら、状態の持ち越しか再描画の競合を疑う
2. **CI のバイナリで再現するか** ── `PKC3_CHROMIUM=…/headless_shell`
3. 再現したら、**test を緩める前に「アプリ側が正しいか」を疑う** ──
   `external-images` の flake は**製品の穴**だった(見張りの登録が user の中身より
   後ろで、解析中の違反を取り逃していた。3 回に 1 回、同意の帯が出なかった)
