...
### ⚠ smoke を同時に 2 本走らせない ── `vite preview` の port が衝突する(2026-09-21)

`npm run test:smoke` は `vite preview` で port を立てる。**同時に 2 本回すと**
片方が port を取れず、`ERR_CONNECTION_REFUSED` が**製品の不具合と同じ顔**で出る
(実測 8 件の偽陽性)。

🔑 **1 本ずつ回す。** 背景で回している smoke が在るなら、**完了通知を待ってから**
次を起動する(`sleep` でポーリングしない ── CLAUDE.md「サブエージェントや CI の
...
ワークフロー(build と smoke を回す)と並べて main のフル smoke を回したら、
**負荷に敏感な 3 件**が落ちた ── 3 件とも **1 本だけで回し直すと緑**だった:

| 落ちた検査 | 何が起きたか |
|---|---|
| `attach.smoke.spec.ts:103` | メインスレッドが止まった時間 109ms(予算 < 89ms) |
| `portable-html.smoke.spec.ts:211` | 書き出した 1 枚に中身が無いまま開いた(15 秒待って)── #1066 |
| `tests/features/manual-page.test.ts`(unit) | 5 秒の時間切れ(既知の #1058) |

🔑 **フルを回すなら、箱の上で他に重い物が走っていない時に回す**(ワークフローの
build / smoke / 全量 unit)。⚠ 並べて落ちたら、**結果を読む前に 1 本で回し直す** ──
緑なら「負荷で出た」と記録し、⚠ **捨てない**(`portable-html` は「重いと中身の無い
1 枚が開く」という製品の順番の問題かもしれないので #1066 に残した)。
...
## 🔴 **押し口を足したら、「押す」検査まで足す** ── 非対称は自分では見えない(2026-09-18、#1006)

新しいボタン(`container-rebuild-run`)を欄に足したとき、spec にはこう書いた:

- **見えている**(`toBeVisible`)/ **押せる**(`toBeEnabled`)/ **説明の 1 行が見えている**

🔴 **押していない。** 一方、**すぐ下の既存のボタン**(`container-reset-run`)は
`clickReal` で押して、窓 → 合言葉 → やめる、まで**最後まで**通っていた。
⚠ つまり**新しく作った動線にだけ、実ブラウザの検査が 1 つも無い**状態で
「smoke は緑です」と報告できてしまう(CLAUDE.md「UI 導線を全量 smoke で誤魔化すな」の実体)。

⚠ **自分では気づけない** ── 書いた側は「足した」ことを覚えているので、
**足りている気になる**。今回は範囲を切って投げた smoke の agent が
「**下のボタンだけ非対称に深い**」と指摘して分かった。

🔑 **機械的な合図**:**押し口を 1 つ足したら、同じ区画の既存の押し口が
どこまで押されているかを読む。** 揃っていなければ、それが足りない分である。

⚠ **押すと本当に壊れる口**(作り直す / 捨てる)は、**最後まで押さない**で構わない ──
🔑 確かめたいのは「**押しただけでは何も起きない**」と「**やめるで戻る**」である。

### ⚠ 「並びも見る」と書いたのに、件数しか見ていない

同じ spec で 2 件目が出た。2 段になった欄にこう書いていた:
...
🔴 **件数しか見ていない** ── 並べ替えても落ちない。
🔑 直しは `.nth(0)` / `.nth(1)` を**画面の字**(`toHaveText`)で見ること
(⚠ 内部の `data-pkc-action` ではなく、**user が読む字**で)。
⚠ そして **DOM の順は画面の上下ではない**(CSS で入れ替わる)ので、
`boundingBox().y` でも比べる ── ただし**字の比較が先に落ちる**ので、
**y の 2 行は変異では通っていない**(そう自覚して置く)。

🔑 検算は当て込み 1 回:**`box.append` の順を入れ替えて、落ちることを見る**
(⚠ `cp` でバックアップを取ってから。`git checkout` は後片付けに使わない)。
...
### ⑤b 一覧を最後の節へ足すと、目次から飛んだときの着地点が変わる(2026-09-21)
