...
> 🔴 **2026-09-16(#971 段④)に、この行の判断を覆した。** ⚠ この doc の他の
> 「ZIP64 は断る」という記述も、同じ日に**実装した**ので既に古い。
> 理由:user の保存領域が 4GB を超えたとき、**書き出しごと断られて持ち出す道が塞がった**
> ── 「実装しない」は**容量が上限に当たる日が来ない**という前提の上に立っていた。
> 実体は `src/features/export/zip-int.ts`(8 バイトの数を 1 本に寄せた)と、
> writer / reader の ZIP64 の欄。⚠ **超えたときだけ**切り替えるので、
> 普通の大きさの書庫は 1 バイトも形が変わらない(古い解凍ソフトで開けるまま)。
