...
- 🔴 **ゴミ箱の版は書き出すが復元しない。** `deleteEntry` は tip を `kind='full'` で
  確定して entry 行だけ消すので、削除済み lid の鎖はバックアップに入る(データは
  失われない)が、復元先に tip が無いので積めない。「これさえあれば全部戻る」とは
  まだ言い切れない ── 復元するには**ゴミ箱 lid の写像**が要り、そこは P6c review H-1
  で「生存 entry だけ見ると事故る」と実証済みの危険地帯なので、独立した段で扱う
