...
断り文の「(入れ先のノートが決まりません)」/ マニュアル「作る・開く」の「いま開いているノートに
...
断り文の「(入れ先のノートが決まりません)」も、マニュアル「作る・開く」の「いま開いている
...
### 🔴 段② の着地前レビュー 2 本が出したもの(2026-09-03。記帳)

**6. 🔴 ④ が、いちばん打つ 4 つの欄に 1px も届いていなかった**(両レビューが独立に指摘)。
素の `textarea`(0,0,1)は `[data-pkc-field='editor-body' / 'append-input' / 'row-source' /
'fm-source']`(0,1,0)に**負ける**。⚠ `@media` は詳細度を上げない ── **この file 自身が
2 か所でそう書いている**。実測:直す前は `editor-body` **12.5px** / `append-input` **12.5px**
(探す欄だけ 16px)= **ノートを書く欄では、直したはずの拡大がそのまま起きていた**。
🔑 `[data-pkc-slot='root']` を噛ませて (0,1,1) にした(field 名を並べないので、
将来の欄も自動で入る)。
⚠ **smoke が緑だった理由**:見ていたのが `entry-filter` **1 個**で、そこは
**field 側に `font-size` を持たない唯一の類**だった(§1「救い手が変わっただけ」)。
🔑 **全数走査**へ直した ── #588 が名指しで戒めている「1 個目だけを見た」形である。

**7. 🔴 折る形にすると、横の受け皿(`overflow-x: auto`)が消える。**
帯に出るのは **`<題名> <経過>` = user の題名そのまま**で `nowrap` なので、長い題名で
**頁ごと横に広がる**(#586 の症状が戻る)。🔑 項目の側で**省く**(`…`)ようにした ──
実測:84 字の題名 + タイマー 1 本で `scrollWidth` **390 = 窓 390**(広がらない)。

**8. 🔴 「入りきらなければ折る」は幅に依らず要る。**
768px(iPad 縦)で 3 本走らせると、直す前は 3 本目の右端が **868px**、
`elementFromPoint` は **null**(押せない)── スマホ用画面の境目(720px)を超える幅なので、
`[data-pkc-layout='phone']` だけに置くと**タブレットが取り残される**。
🔑 素の規則を `flex-wrap: wrap` にした(広い窓では 3 本でも 1 行に収まるので見え方は不変)。

**9. 🔴 折ると本文を 0 まで潰せる。** 版面の本文の行は `minmax(0, 1fr)` で**下限が 0**。
1 件 1 行にすると走っている数だけ線形に食い、**上限が無い**(§2 の「合計が画面を
超えたら document が縦スクロール = 本文を潰さない」と**順番が逆**)。
🔑 スマホでは帯の丈を **25dvh** で頭打ちにし、その中で縦に送る。

**10. ⚠ 数字を足し算で推定して書いていた** ── 「押し所は 20px 弱」は誤りで、**26px**
(素の `button { height: var(--row-h) }` + `--row-h: 26px`)。お知らせ・CHANGELOG・
マニュアルの 3 か所へ伝播していたので全部直した。
⚠ そして **`opacity: 0` の要素は押せる場所に残る** ── 直す前は「指では出ない」ではなく
**「見えないのに押せていた」**(囲みの右上に触ると黙ってコピー、図なら絵が原文へ化ける)。
🔑 user から見た実害はこちらのほうが大きいので、お知らせにも 1 行入れた。

**11. ⚠ 門を N 個置いたら、N 個目だけが鳴る場面を N 通り。**
丈は帯 **3 本**に、折る規則は **2 つのリスト**に置いたのに、実ブラウザで押しているのは
タイマーだけだった(収録の帯を落としても緑 ── **マイクを止める唯一の出口**である)。
🔑 録音は許可が要って高くつくので、**選択子の集合を構文で pin** した。

**12. ⚠ 測ったブラウザを書く**(CLAUDE.md §5)。`hasTouch` で `(hover: none)` /
`(pointer: coarse)` が両方真になることは、**2 つのバイナリ両方で確かめた**(2026-09-03):
同梱のフル Chromium(`/opt/pw-browsers/chromium`)と、CI の実体である
`chromium_headless_shell-1194`(`PKC3_CHROMIUM=…/headless_shell`)── どちらでも
`touch.smoke.spec.ts` の 7 件が緑。⚠ 片方だけだと `afterprint` の事故と同じ形になる。

### 🔴 段③ を実装して分かったこと(2026-09-03。記帳)

**13. 🔴 この doc が書いた観測点が、直す前から真だった。**
段③ の行には「**375×667 で 2 ペインの「移す」が両ペインで押せる**」と書いてあるが、
規則を外して測り直したら**直す前も押せていた**(`elementFromPoint` は `dual-move`、
y=634 / 窓 667)── 操作行は最下段の全幅なので、上の 2 枚をどう割っても位置が変わらない。
🔑 **実害は別の所に在った**(375px の窓、規則を外して実測):

| | ペインの幅 | 見出し帯 `scrollWidth`/`clientWidth` | パンくずの幅 |
|---|---|---|---|
| 直す前(`1fr 1fr`) | **184px** | **187 / 182**(はみ出す) | 🔴 **0px** |
| 直した後(上下) | **375px** | 373 / 373 | 182px |

⚠ つまり「**いまどちらの箱に居るか**」が 1px も見えないまま「移す」を押すことになる。
🔑 だから smoke が見るのは**パンくずの幅**と**見出し帯がはみ出さないこと**にした
(「押せるか」は対照群として残す ── **それだけでは変異が生き延びる**)。
⚠ 一般形:**観測点を先に書いたら、着手のときに「直す前から真ではないか」を測る。**
書いた本人が正しいと思っているので、測らないと誰も検算しない。

**14. 🔴 `install()` の `tellTooNarrow()` は no-op だった**(変異試験 M7 が SURVIVED)。
`appPhone.install` は `main.ts:730`、帯の口を配る `setFoldNotify` は `:989` ──
**install の時点で聞いている人は 1 人も居ない**ので、呼んでも必ず素通りする。
🔑 鳴らすのは `onTooNarrow`(**繋いだ瞬間**)1 か所へ寄せ、install 側は**消した**
(CLAUDE.md「『これが無いと壊れる』と書く前に、外して壊れるのを見る」)。

**15. 🔴 「1 度だけ」は実ブラウザでは原理的に測れない。**
状態の行は **1 行しか持たない**ので、同じ字を 2 回書いても画面は 1 バイト違わない。
🔑 だから「1 度だけ」は unit(`tests/adapter/phone-layout.test.ts`)が替え玉を
4 回動かして数え、smoke が持つのは実ブラウザにしか無い 2 つ ──
①`matchMedia` が本当に真になって帯へ届くか ②**画面が止まっていないか**(裁定の後半)。
⚠ そして **360px ちょうどの対照群**を別の test に置いた ── 置かないと、
境目を `PHONE_MIN_PX - 1` から広げる変異が**そのまま通る**。

**16. ⚠ 黙る条件が正反対の 2 つを、同じ口へ通した。**
`sayFolded` は**スマホで黙り**、対応外の 1 行は**スマホより狭いときだけ鳴る**。
🔑 それでも口は 1 つ(`setFoldNotify`)にした ── #606 が直した欠陥は
「口が 2 つあって、片方を配線し忘れた」であり、寄せておけば
**落としたときに畳みの知らせも同時に消える**(既に在る smoke が鳴る)。

**17. 🔴 この doc の予告どおり `read-columns` の 700px の腕が落ちた。**
§6 に「段③で fold-notify をスマホで黙らせると落ちる」と書いてあり、実際に落ちた。
🔑 直し方は**幅を 800px へ上げる**(器は 912px を割るが版面はパソコンのまま)+
**700px のスマホ側を対照群として新設**する ── 片側だけ書き換えると、
「黙る」を守る検査がどこにも無くなる。⚠ 対照群では
**本当に畳まれていること**も同じ it で assert する(黙るだけなら、何も配線しなくても真)。

### 🔴 段③ の着地前レビュー 2 本が出したもの(2026-09-03。記帳)

⚠ **7 件が本物だった**(実装 3・動線 4)。うち **5 件は自分の実測で確定**し、
2 件(横向き / 帯のはみ出し)は**レビューが「測っていない」と断って出した**ものを
こちらで測って**両方とも当たっていた**。

**18. 🔴 直した結果が、横に持ったスマホでは正味で悪化していた。**
「スマホなら積む」と書いたので、**667×375(横)でも積んでいた**。測ったら:

| 窓 | 並べ方 | 1 枚 | パンくず | 見える行 |
|---|---|---|---|---|
| 667×375 | 左右のまま | 330×279 | 136px | **6 / 6** |
| 667×375 | 上下にすると | 667×136 | 474px | 🔴 **1 / 6** |

⚠ 横では**左右のままでもパンくずが読める**ので、直す理由がそもそも無い ──
積むと「どこに居るか」を買って「**何が在るか**」を失う。
🔑 条件は **`@media (orientation: portrait)`**(数字を 1 文字も書かないので、
「幅の数字は TS の 1 か所」を破らない)。
⚠ 一般形:**直した幅で測って満足しない。** 同じ規則が当たる**別の形**
(縦 / 横、大きい / 小さい)で 1 度ずつ測る。

**19. 🔴 `decl('grid-template-columns', '1fr')` は `1fr 1fr` にも当たる。**
`decl` は「値の頭」しか留めないので、⚠ **直す前の `1fr 1fr` へ戻す変異が
4 つの assert 全部を通り抜けた**(レビューが再現つきで出し、こちらで R1 も取った)。
🔑 **末尾の `;` まで留める**(`'1fr\\s*;'`)+ **`1fr 1fr` に当たらないこと**を
1 行足す。⚠ これは §1 の「救い手が変わっただけ」ではなく、
**主張(列が 1 本)と検査(1fr で始まる)がずれている**型である。

**20. 🔴 共有の口に門を置いたら、通る 2 種類のうち片方で理由が消えた。**
`sayFolded` に `if (appPhone.isPhone()) return;` を置いたが、この口を通るのは
2 つで、黙ってよい理由は**片方にしか無い**:

| 呼び元 | 文言 | スマホで黙ってよいか |
|---|---|---|
| `read-columns.ts` | 「幅が足りないので段組みをやめました」 | ✅ 1 枚ずつ出すのが既定 |
| `split-view.ts` | 「幅が足りないので、横に並べる枠を N 枚畳みました」 | 🔴 **駄目** ── 本当に幅で落ちている |

⚠ 後者は `fittingSplitFrames` が 1 枚あたり約 448px を要求するので、
スマホの幅では **2 枚目が絶対に入らない** ── 黙らせると user が押した
**「このノートを横に留める」が無言で効かない**(#583 で直した dead click と同じ形)。
🔑 **黙る判断は呼び元が持つ**。口は 1 つのまま(#606 の規律は保つ)。
⚠ そして**お知らせに書いた理由が、その場合には嘘だった** ──
「幅のせいで畳んでいるのではありません」は枠には当てはまらない。着地前に直した。

**21. 🔴 「1 度だけ出す」を「一度出したら二度と消えない」と読んでいた。**
動線レビューの最重指摘。⚠ 窓を 800px に広げても字が残り、
**対応している幅で「対応していません」と書いてある** = 画面が嘘をつく。
しかも `noticeLine` を空へ戻す経路は **file 全体で 0 件**、行に「×」も無いので
**user にできることが 1 つも無い**。⚠ これは `main.ts` が #300 段④ で
常設バッジを外したときの理由(「1 行しかないので本当に読ませたい文を押し出す」
「user がそこでできることは何も無い」)を**両方満たしている**。
🔑 直し方は「知らせ」ではなく **状態を配る**こと ── `onTooNarrow(true/false)`。
「1 度だけ」は `lastTold` が保つ(同じ値を続けて配らない)。

**22. 🔴 いちばん狭い画面へ向けて、その画面でいちばん長い字を出していた。**
`TOO_NARROW_TEXT` の末尾に足した「(画面は止めません)」は、
実測で **340px・11px の帯に収まらなかった**(`scrollHeight 25 / clientHeight 20`)。
⚠ 帯は `height: 20px` 固定で `overflow` を持たないので、**画面の外へ落ちる**。
外すと **20 / 20** でぴったり収まる ── 外した版は **user 裁定 ⑥ の言葉そのまま**である。
🔑 打ち消し(「止めません」)は**疑いを生む**ので、そもそも user の言葉ではなかった。
⚠ そして `toContainText` は**画面の外の字でも真になる** ── smoke に
`scrollHeight <= clientHeight` を足した。

**23. ⚠ 積み方を変えたら、呼び名が画面と逆になった。**
`aria-label` は「左のペイン」「右のペイン」のままだったので、
上下に積んだ画面を**逆に読み上げていた**。🔑 判定は **DOM の実寸 1 か所**
(`stackedNow`)から採る ── `matchMedia` で聞き直すと、積むかどうかを決める口が
CSS と JS の 2 つになる(CLAUDE.md §7)。
⚠ 「移す」の**向きを画面に出す**かどうかは見え方の話なので、user へ出す。

**24. ⚠ `onTooNarrow` の中の `if (this.saidTooNarrow) return` は no-op だった。**
`tellTooNarrow` の先頭が同じ条件を見るので、論理的に通らない
(どの test も真の側を 1 度も通らない)。🔑 消した ── §1 の
「『これが無いと壊れる』と書いた規則が no-op」と同じ型で、**2 度目**である。

### 🔴 段④ ── マニュアルを書く前の実地調査が、嘘を 1 つ止めて欠陥を 1 つ掘った(2026-09-03。記帳)

**25. 🔴 「スマホでは鍵が効かない」と書くつもりだった。実装では効く。**
段④ の行(§5)は「マニュアル「画面を組み替える」 に…**鍵は画面に効かない**…」と書いてあり、
私はそのまま写すつもりだった。⚠ 調べたら **`runGlobalCommand` に版面の門は 1 つも無い**
── `visibility: hidden` + `inert` の面の中のボタンも、委譲の `click` は root まで
上がるので**押せる**(両ブラウザで実測)。`Ctrl+F` は `reveal('list')` を通って
**ページを移してから**焦点を入れる(unit が守っている)。
🔑 **「効かない」は「効く」より強い主張**なので、書いていたら誰も検算しないまま残った
(CLAUDE.md「できないは、できることより強い主張」)。
⚠ 実装では決まらないのは「**ソフトウェアキーボードに修飾キーが無い**」ことだけなので、
マニュアルはそう書いた ── **環境の話と実装の話を混ぜない**。

**26. 🔴 `Mod+Alt+\`(集中モード)が、スマホで無言のまま保存値だけ動かしていた。**
実測(375×667):

| | `pkc3.panes` | 状態の行 | `data-pkc-hidden-panes` |
|---|---|---|---|
| 押す前 | `null` | 空 | `null` |
| `Ctrl+Alt+\` の後 | 🔴 **`'sidebar inspector'`** | 🔴 **空のまま** | `null`(画面は 1px も動かない) |
| 対照群 `Alt+[` | 変わらない | 「…列は畳めません」 | ── |

⚠ 段① は `toggle-pane` にだけ門を置き、**対称の反対側**を取りこぼしていた ──
しかも `toggle-pane` の門のコメント自身が
「**保存値も動かさない ── 動かすと、PC の幅へ戻したときに身に覚えのない畳みが残る
(見えない状態変化)**」と、**この害を名指しで戒めていた**。
🔑 CLAUDE.md「**片側を直したら、対称の反対側を必ず疑う**」の 4 例目である。
⚠ 検査は 2 つで 1 組:**理由が出ること**と 🔴 **保存値が動かないこと**
── 後者を書かないと「断ってから畳む」実装が素通りする。

**27. ⚠ 段④ の予定表そのものが腐っていた。**
「#588 を閉じる」は**もう閉じていた**(2026-09-02、PR #664)。
「#586 を閉じる」は**段② が別の道で直していた** ── #586 の本文は
「答えの候補は #587 の改善 B なので**単独では直しません**」と書いていたが、
段② が「1 件 1 行に折る」で潰しており、`timer.smoke.spec.ts:118 / :213` が
**3 本ぶんの止める口を全数走査**して守っている。
🔑 CLAUDE.md「**着手の前に issue の本文を 1 件 1 grep で検算する**」は、
**自分が書いた予定表**にも当てる。

**28. 🔴 マニュアルに「在る物を無い」と書いた ── しかも同じ file の 20 行下で自分が反証していた。**
§4-0 へ足した 1 文は「**スマホではこの帯そのものが出ません**」だったが、CSS が消しているのは
**左右の帯だけ**(`[data-pkc-region='pane-grip']:not([data-pkc-axis='y'])`)で、
**追記欄の帯は残っている** ── その規則の直前のコメント自身が
「**横向きの帯(追記欄)は残す ── 消すと畳んだまま二度と戻せない**」と書いている。
⚠ しかも**同じ PR で書いた新しい節が「追記欄を畳む(`Alt+\`)は効きます」と書いていた** ──
**1 つのマニュアルの中で矛盾していた**。
🔑 「無い」と書くときは、**消している規則の選択子を最後まで読む**
(`:not(...)` を読み飛ばすと、範囲を実際より広く読む)。

**29. ⚠ 画面に無い呼び名を、お知らせ・CHANGELOG・マニュアルの 3 か所へ同時に作った。**
「**集中モード**」という字は製品のどこにも無く、画面と設定の label は
「**両側のペインを畳む / 戻す(集中)**」である(`features/keymap.ts:469`)。
⚠ お知らせを読んだ user が ⋯ →「操作を探す」で「集中モード」と打つと **0 件**になる。
🔑 **user に配る字は、画面の字から引く**(新しい呼び名を発明しない)。

**30. ⚠ 断られる鍵の綴りを半分しか書いていなかった。**
`toggle-sidebar` の既定は **`Alt+[` と `Ctrl+\` の 2 つ**(`keymap.ts:428`、note に
「PKC2 の `Ctrl+\` と同じ手」)。⚠ §10 は両方載せているので、新しい節だけ片方欠けると
**「自分が押した鍵は別の不具合か」**と読まれる。

**31. ⚠ 内部の言葉が 2 つ残っていた。**「全ページで数えて確かめています」(誰が数えたかは
user の関心ではない)/ Office の「実機ではまだ確かめていません」(読んでも user にできることが
増えない → 「押すと足りないものが名前で出ます」に置き換えた)。

⚠ **レビューの 6 件中 1 件は誤りだった**(「2 ペインで整理はスマホでは別タブが開いて PKC から
出る」)── `tiles.ts:189` のコメントが `kind: 'dual'` を「**PKC をもう 1 枚ではない**」と
明記しており、実測でも面(`pane` ページ)として開く。
🔑 **file:line 付きの断定ほど検算を飛ばしやすい**(CLAUDE.md)── 1 grep で反証できた。
...
| 段③ 面の残り ── ⋯ に `NOTE_TOOL_ACTIONS`(添付 / 録音 / 画面収録 / 計測)+ create-bar との parity pin(**段① へ前倒し済み**)/ 対応外(<360)の 1 行 / `focus-search` `filter-by-tag` `toc-jump` が `phoneReveal` を呼ぶ(**段① へ前倒し済み**)/ 2 ペインを上下に / fold-notify はスマホで黙る / print.smoke に 559px の腕 | M | ✅ | probe:本文ページで sidebar にしか無かった note-scoped 4 つ + パレット + 右クリック 11 = 16 種が ⋯ から届く(名前で等値 pin)/ 390×844 で本文のタグ札を tap → 一覧ページが絞られた状態で出る / 情報ページの目次を押すと本文ページに戻り見出しが画面内 / 340px で状態の行に対応外の 1 行が 1 度だけ / ~~375×667 で 2 ペインの「移す」が両ペインで押せる~~ 🔴 **これは直す前から真だった**(上の 13)── 実際の観測点は **375×667 で 2 ペインのパンくずの幅が 0px でない**(直す前 0px / 直した後 182px)|
| 段④ 届ける ── マニュアル「スマホで使うとき」の節(ページ 3 つ・帯・⋯・鍵は画面に効かない・対応外・Office は保証外)/ §4-0「帯は残る」をスマホの例外つきに / お知らせ / CHANGELOG / #609 に「スマホ用画面で消えたもの / 広い窓で残るもの」を記帳し #588 #586 を閉じる | S | ✅ | docs-parity 緑(set-view 等値はそのまま / CHANGELOG 等値)/ #609 のコメントに 2 列の表(消えた 3 件 / 残る:721〜1100 の右の掴む帯なし・1280 で畳むとパレット 0) |
...
- 🔴 横向きのスマホ(844×390 / 667×375)は幅が 720 を超えるので 2 列版面に落ちる ── 裁定は「幅」なので設計では触らない。🟢 **段⓪ の計器で数字が出た**(844×390 で「操作を探す」がカードを閉じても押せない。押せない物が**横向きの 1 件だけ**に出ているのが、幅の話ではない根拠)ので **#663 として起票済み**。「高さでも切るか / 2 列版面の本文に下限を置くか」は、そちらで画面の言葉の設問にする。
  - ✅ **2026-09-04、#663 は「高さでも切る」で着地**(推薦 ①):`PHONE_MAX_HEIGHT_PX = 480`(`features/phone-layout.ts`)を足し、`matchMedia` の query を `(max-width: 720px), (max-height: 480px)` の OR にした。数字の正本は features 側 1 か所のまま、CSS には書かない(§7 は守れている)。直す前の計器:844×390 / 800×400 で「操作を探す」が 4 行とも押せない(カード開 = announce、閉 = inspector に覆われる)。1280×500(低い PC 窓)は境目の外側で 2 列版面のまま
...
## 🔴 撤回の記帳 ── 2 ペインの「上下に積む」は 2026-09-04 に取り下げた(#671)

⚠ **この doc の中で「2 ペインファイラは上下に積む」「条件は `@media (orientation: portrait)`」
「呼び名を実寸から採る(`stackedNow`)」と書いてある所は、もう実装と違います。**

user 裁定 2026-09-04(#671、逐語):

> **「左ペイン表示と右ペイン表示に分けて、どちらかを開いている際は、もう片方の行き先だけ示す。
> みたいな簡易 UI にすればよいのでは?意図した機能はフォルダ整理の体験向上なのだから、
> それでも良くないか?」**

| | 撤回した形(2026-09-02) | いまの形(2026-09-04) |
|---|---|---|
| 並べ方 | 縦は上下に 2 枚 / 横は左右に 2 枚 | 🔑 **1 枚だけ**(向きで分けない) |
| 隠し方 | ── | `visibility: hidden` + `inert`(`display: none` にすると見ていた場所が戻る) |
| 呼び名 | 実寸から「上 / 下」を採る(`stackedNow`) | **「左 / 右」1 本**(`stackedNow` は削除) |
| 行き先 | ── | 「右のペインへ →」+ 操作の字に行き先(「右へ写す」) |

🔑 **撤回の理由は「積む案は向きで得失が反転する」**である ── 縦は得だが横は 6 行のうち
1 行しか出ない。1 枚ずつならどちらの向きでも版面を丸ごと使うので、向きで分ける理由が消える。

実装:`src/styles/app.css`(スマホの 2 ペインの節)/ `src/adapter/ui/render/dual-filer.ts`。
検査:`tests/adapter/phone-layout.test.ts` の「2 ペインを向きで分ける規則は残っていない」。
