...
渡すのが早すぎると外殻がまだ聴いていない。⚠ **取りうる形は 2 つで、どちらも未検証**:

| 形 | 中身 | ⚠ |
|---|---|---|
| (a) **届くまで送り直す** | 一定間隔で port を渡し、**港越しの返事**が来たら止める | 何回・何秒で諦めるかを決める必要がある |
| (b) **外殻に控えを置く** | 外殻の script が最初に `window.name` 等の同期の印を読む | 同期に読める場所が要る(未調査) |

🔑 **段① の最初の実験はここである** ── 決め打ちで実装せず、**まず (a) を測る**
...
- **§4 の港の渡し方**:(a) を測って届かないなら (b) へ。⚠ どちらも未検証である
