...
> 状態:**設計のみ。実装は 1 行も入れていない**(doc-first)。§5 の 7 問は **2026-09-25 に全問裁定済み**(§11)。§4 は推薦で進める(委任の内側)。
...
| **C14** | N2 | P2 | 一覧・フォルダ表・2 ペインの行でノートを 2 回続けて押すと、別のウィンドウ(付箋)で開く(2 回押しの判定は 1 か所 `binder.ts:11455-11457` なので 3 つとも同じ)。フォルダの 2 回押しは今までどおり「入る」 | 押した結果 | **Q5** | #1042 | S:`binder.ts:11442-11454`(フォルダ以外 → `open-note-window` の受け手を同じ lid で撃つ)/ unit + smoke assert 1 つ |
...
### 3.1 見本(実物の PKC3 main `ecff727` を build し、画面に CSS / DOM を注入して撮った。製品のソースは変えていない)

⚠ 見本は「案がこう見える」を示すための物で、字の大きさ・色・余白は**いまの実物の規則のまま**です(見本のために目立たせていない)。

| 見本 | どの案 | 見るところ |
|---|---|---|
| ![右クリック(推薦 C)](images/ledger3-mock-context-menu.png) | **見本 1** C1 / Q1 の C(推薦) | 「このノート」のまとまり(履歴 / 名前を変える / 移す…)の先頭に「編集」+ Ctrl+E。写す・開く・書き出すの並びは今と同じ。まとまりの間が空く |
| ![右クリック(A)](images/ledger3-mock-context-menu-top.png) | **見本 1b** C1 / Q1 の A | メニューの先頭に「編集」(2026-09-04 の裁定を覆す形)。写す 2 つ以下が 1 つずつ下へずれる |
| ![編集中(案)](images/ledger3-mock-editing-status.png) | **見本 2** C4 / Q2 | 画面の**いちばん下の 1 行**の左端に「編集中」、追記欄の所の文が「編集中」の 1 語に。⚠ いまのステータスバーの字(小さく薄い)のままなので**かなり目立たない** ── 気づきにくければ色を付ける(§9 の C4) |
| ![編集中 + 他の知らせ(案)](images/ledger3-mock-editing-status-mixed.png) | **見本 3** C4 / Q2 | 「編集中」と保存先の注意(実物の字)と「保存中…」が同じ 1 行に並んだとき。画面では右端で切れるので、下に切り抜きを付けた |
| ![編集中 + 他の知らせ(切り抜き)](images/ledger3-mock-editing-status-mixed-zoom.png) | 見本 3 の切り抜き | 先頭は常に「編集中」 |
| ![一覧の選び足し(案)](images/ledger3-mock-list-marks.png) | **見本 4** C13 / Q6 | 「会議メモ」を開いたまま「買い物」を選び足した所(中央は会議メモのまま)。⚠ 選び足した「買い物」の行の背景は、**フォルダの表のいまの濃さのままだとほとんど見えない** ── Q6 の注記 |
| ![2 ペインの下の列(案)](images/ledger3-mock-dual-bar.png) | **見本 5** C12 ④ / Q4 | 画面下の 1 本の列(左右のペインで共通)の「名前」→「名前を変える」、「ゴミ箱」→「削除」 |
| ![2 ペインの下の列(切り抜き)](images/ledger3-mock-dual-bar-zoom.png) | 見本 5 の切り抜き | F キーと並びは今のまま。「Shift + F4 ノ…」が切れているのは今の実物のまま(この案とは別) |
| ![設定の節(案)](images/ledger3-mock-settings-toggles.png) | **見本 6** C18 / Q7 | 本文の置き場所 / 文字の大きさ / 本文の段組み / 段の境界線 の 4 項目がボタンの列に、各項目の説明が 1 行に。⚠ 見本で替えたのは 4 項目だけ(案は 10 項目)── 写っている「本文のタグの見せ方」「編集の仕方」も案ではボタンの列になる。配色 / ページ設定 / 貼り付け元 はプルダウンのまま |

### 3.2 まだ設計していない物(⛔ 未設計・後日提示 ── 上の表とは別。ここでは聞かない)

| id | 行 | 原則 | 方向(決まっているのは #582 §3.3「採る」だけ) | 次にやること |
|---|---|---|---|---|
| **C7** | G7 | P5 | 見出しの右クリック「ここから編集する」が、その章だけを欄で開き、その間も他のノートや横の枠は触れる ── **どう見えるかは未設計** | 設計 doc 1 本(#1044)。「1 画面の行の欄が phase を取らずに書けるか」の実測から。⚠ 合成 lid は採らない。量 L:`app-state.ts` / `binder.ts:2303` / `detail.ts` / `row-swap.ts` |
...
⚠ **2026-09-25 に全問裁定済み ── 結果は §11。** 以下は聞いたときの形のまま残す。
...
- 注記:見本 4 のとおり、フォルダの表で選んだ行の背景は**いまの濃さだとほとんど見えません**(`app.css:6358-6360` の `var(--surface-2)`)。A を選ぶ場合、濃さもフォルダの表と一緒に上げるかどうかを合わせて伺います(一覧だけ濃くすると、同じ物の見え方が 2 つになるので、上げるなら両方)。
...
新しく切った issue(#1038 の子。トラックにしない。2026-09-25 起票):

| issue | 題 | 中身 3 行 |
|---|---|---|
| **#1042** | 一覧をキーボードとマウスの近道で辿る(C2 / C3 / C14) | ①一覧タブに矢印・Enter・絞り込みからの降下・打ち替え後の焦点の戻し ②Escape で 1 段閉じる + 別ウィンドウも Escape(`global` でない文脈 2 つ、検算は緩めない。付箋は閉じ道が別なので段を分ける)③ノートの 2 回押しで別ウィンドウ。関連 #1032 / #257 / #240 / #332 ── #257 が「#240 と一緒に」と先送りし #240 が受け取らなかった 2 度目の未着手であることを書く |
| **#1043** | 編集中でも、開かずに済ませる道を止めない(C6) | ①`TOGGLE_TASK` ほか 7 case と `bodyRewriteGate` を lid で判定する門へ ②同じ lid のときは既存の `EDITING_NOTE` で断る(無言 0 件を全数 pin)③書込の門なので変異試験を必ず当てる |
| **#1044** | 章だけ編集・章の別ウィンドウ・コード枠のその場編集の設計(C7 / C8) | ①まず「1 画面の行の欄が phase を取らずに書けるか」を実測 ②phase を変えない章の状態を設計(合成 lid は採らない)③設計 doc 1 本を画面の言葉で 1 度で見せる。#582 §3.3 の設計として起票 |
| **#1045** | 修理の束(C9 / C10 / C11 / C15 / C16 N4 / C17 の記録) | ①C9 は先に実ブラウザで再現 ②C16 は本物の build で測ってから ③1 PR 1 件で単独に着地、お知らせ 1 行。C11 は C4 / C6 より先 |
| **#1046** | ボタンの名前を「何を + どうする」にそろえる(Q4 の裁定) | ①全部のボタン・メニュー項目・鍵の一覧の字を目録にし、規則に合うかで分ける ②変わる字の一覧を画面の言葉で見せてから入れる ③#1017 §6.1 規則 1「ボタン = 動詞だけ」を改める(C12 はこれに置き換わった) |
...
| 段 | 何を | 裁定 | 量 | 着地条件 |
|---|---|---|---|---|
| **A** 修理の束 | **C11(断り文を 1 か所へ)を最初に** → C9(先に再現)/ C10 / C15 / C17 の記録 → #1045 | 不要 | S〜M | 1 PR 1 件。お知らせ 1 行。C11 が入るまで C4 / C6 に着手しない(断り文が一時的に 4 系統にならないため) |
| **B** キーボードの床 | C2 → #1042 | 不要 | M | keymap の検算を緩めない / smoke は既存の台に assert |
...
| **F** 閉じる鍵・2 回押し | C3(予定表・連絡先 → 付箋の閉じ道は次の PR)/ C14 → #1042 | **Q3 / Q5** | M | `KEYLESS` から外す / 新しいお知らせ 1 件 / `CONTEXT_LABELS` 2 行 |
| **G** 一覧の選び足し | C13 | **Q6** | M | 背景の CSS はフォルダ表の規則を共有(2 本にしない)/ 中央が切り替わらないことを unit で pin |
| **H** 編集中に止めない | C6(A の C11 の後)→ #1043 | 不要 | M | reducer の無言 0 件を全数 pin + 変異試験 |
| **I** 章だけ編集の設計 | #1044 の doc → 裁定 → C7 → C8 | doc の後 | L | 「1 画面の行の欄が phase を取るか」の実測から始める |
...
選択式の設問で 2 回に分けて伺った(Q1〜Q4、Q4 の確認 + Q5〜Q7)。

| 問 | 裁定 | この doc への影響 |
|---|---|---|
| Q1 右クリックの「編集」 | **C** ── 「このノート」のまとまり(履歴 / 名前を変える / 移す…)の先頭に「編集」。まとまりの間を空ける | C1 は見本 1 の形。2026-09-04 の裁定(毎日使う項目を下へずらさない)はそのまま生きる |
| Q2 「編集中」 | **A** ── 画面いちばん下の行の先頭に「編集中」、追記欄の注意文は 1 語に。保存 / キャンセルは上も追記欄も残す | C4 は推薦どおり。気づきにくければ色を付ける(§9) |
| Q3 Escape | **A** ── 何も編集していないとき Escape でノートを閉じる + 予定表・連絡先・付箋の別ウィンドウも Escape で閉じる | C3 は推薦どおり(#1042)。9/21 のお知らせは書き換えず、新しいお知らせを 1 件出す |
| Q4 字をそろえる | 🔴 **個別の 4 件ではなく、名前の付け方の規則** ── 下の「Q4 の中身」 | C12 は #1046 に置き換わった。#1017 §6.1 規則 1 を改める |
| Q5 2 回押し | **A** ── 一覧・フォルダ表・2 ペインでノートを 2 回続けて押すと付箋で開く | C14 は推薦どおり(#1042) |
| Q6 一覧の複数選択 | **A + 濃く** ── 一覧タブでも Ctrl / Shift で選び足せる。選んだ行の背景は**フォルダ表と一緒に**いまより濃くする | C13 に「濃さを上げる」を足す(同じ物の見え方を 2 つにしないため、フォルダ表・2 ペインも同時に) |
| Q7 設定の形 | **A** ── 選択肢 4 つ以下の 10 項目をボタンの列に | C18 は推薦どおり |

### Q4 の中身

設問「同じ操作なのに場所によって違う字を、ボタンの字に揃えてよいですか」への答えは、個別の揃えではなかった:

> 動詞と名詞を使い分けてverb-nounにすべき(逐語)

確認の設問で、次の読みで合っていると裁定された(こちらの言葉で):

- **押すと何かが起きるボタン** = 「**何を + どうする**」。例: 2 ペインの「名前 / ゴミ箱」→「名前を変える / ゴミ箱へ移す」、鍵の一覧の「編集を確定する」→「編集を保存する」
- **見出し・タブ・場所の名前**(一覧 / フォルダ / システム)= 名詞のまま
- 待ちの字は対象を残す(「本文を読み込んでいます…」)
- 左上の「+ ノート」は + が「作る」の印なのでそのまま(user 指示 2026-08-05「+〇〇みたいにボタンを変更」の形)
- 見出しの下のボタンも「何を + どうする」(「見出しの下だけ動詞のみ」は選ばれなかった)
- 🔴 **全部のボタンの名前をこの規則で数え直し、変わる字の一覧を見せてから入れる**(#1046)

⚠ これは `ui-total-design-2026-09.md` §6.1 規則 1「見出し = 型の名前、ボタン = 動詞だけ」を改める裁定である(同 doc に注記した)。規則 3(「〜して、〜する」の動詞句をボタン名にしない)と規則 5(同じ型の同じ操作は 1 つの名前)は生きている。
⚠ 字が長くなる方向なので、#1029 段 C で踏んだ「右の列の段数が増える」を 8 幅で測ってから入れる。
