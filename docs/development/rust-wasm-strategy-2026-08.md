...
> §12 の推奨がそのまま採用された。R0 / R1 は実施済み、**R2 は事前登録した指標により棄却**(§10.5)。
...
| parity | TS を oracle に **7 ケース群 / ランダム 300 本の鎖**で byte 一致(ja / CRLF / 絵文字 / 末尾改行なし / full 混在 / 空 を必須次元に含む) |

### R2: 復元チェーンの wasm 化 ── ❌ **棄却(①実測で逆効果)**

出力は TS と完全一致した(parity 全 green)。**速度が条件を満たさなかった**:

| fixture | 段数 | TS | wasm | 判定(分母 = TS) |
|---|---|---|---|---|
| ascii 2500 行(約 30KB) | 5 / 20 / 50 | 0.85 / 2.85 / 6.28ms | 0.56 / 1.71 / 3.85ms | **34 / 40 / 39% 短縮** |
| **ja 2500 行(約 100KB)** | 5 / 20 / 50 | 0.75 / 3.19 / 7.06ms | 1.48 / 3.65 / 7.94ms | **97 / 15 / 13% 悪化** |

- 手法: 同一プロセス・同一 fixture・交互 5 run の中央値。先に出力一致を確認してから計測(違うものを比べない)。Node 実測でありブラウザ実測ではない(§3.1)
- 原因は §3.2 / F2 の予測どおり **境界の UTF-16↔UTF-8 変換**(とくに出力 decode)。段数を増やすと固定費が償却されて差は縮むが、ja では 50 段でも TS を上回らない
- **PKC3 は日本語が主用途**なので、ascii だけの勝ちでは採用理由にならない。事前登録の規則(「TS を下回ったら棄却」)に従って**本番配線を外した**
- 絶対値はいずれも 1 桁 ms で、体感差を生む領域ではない ── 複雑さを増やす理由がさらに無い

**残したもの**: crate / 橋 / parity test / ビルドスクリプト / pin。次の候補(§4.3 の圧縮・索引構築 ── **出力が小さく計算が重い** = B1/B2 を素直に満たす)を載せるときの固定費は償却済みになる。
**本番の import は 0** なので、現時点で製品 bundle に `.wasm` は入らない。
