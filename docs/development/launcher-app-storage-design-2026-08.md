...
**確定(P8 段⑭ で実装)**: **アプリの保存データは書き出しに含めない**。

保存先は外殻(= アプリ origin)の `localStorage` の `pkc3.app.<lid>.*` で、
container にも添付にも入らない ── **何もしなければ自然に含まれない**。
だから「取り込むかどうかを聞く」ダイアログも要らない(聞く対象が発生しない)。
⚠ 逆に言うと、**バックアップから戻してもアプリのデータは戻らない**。
これはマニュアル「ランチャー」 に明記した。

将来オプトインを足したときの印の扱いは、そのとき改めて決める
(この設計ではオプトイン自体が無い)。
...
## 5. 実装したときに分かったこと(P8 段⑭。この doc の訂正を含む)

- 🔴 **`for...in` を使ってはいけない**(この doc に無かった罠)。target に
  `Storage.prototype` を継がせるので、`for...in` が prototype の列挙可能メンバまで
  舐め、`target.length` の getter が**実 Storage でない `this`** で呼ばれて
  `TypeError: Illegal invocation` になる ── **直す前とまったく同じ「1 行目で死ぬ」**
  に戻る。own の文字列キーだけが要るので `Object.keys` で足りる
- 🔴 **prelude の中でバッククォートを使わない**。この doc の warning は「外側の
  テンプレートリテラルと衝突する記法を使わない」だったが、実装中に**注意書きの
  コメント自体に** `` `target.length` `` と書いてリテラルを閉じ、build が壊れた
- 🔴 **不在の assertion をコメントで汚さない**。shim のコメントに
  「allow-same-origin」の語を書いたら、`expect(html).not.toContain('allow-same-origin')`
  が落ちた ── test が正しく鳴った側の例
- 🔴 **`appId` を渡す形と渡さない形で出力が別物**になったので、既存の隔離の pin が
  **渡さない呼び方に救われて空振り**になった(「外殻は script を持たない」が
  `appId` 無しで緑のまま)。test 側を本番と同じ形に寄せた
- ⚠ **保存だけでは SPA は動かない**。同時に必要だったもの:
  ① `<base>`(blob: は opaque path なので `new URL(相対, base)` が `TypeError`。
  ⚠ **`about:srcdoc` でも同じく落ちる** ── 階層 URL でなければ意味が無い。
  ⚠ **origin の根にもしない** ── `assets/…` が PKC3 自身の資産に解決する)
  ② `allow="clipboard-write"` ③ 文字コードの判別(`blob.text()` は UTF-8 固定で、
  Shift_JIS 保存の HTML が不可逆に化ける)
- ⚠ **`history.pushState` は「全滅」ではない**。**解決後の URL が document URL と
  fragment 以外で一致すれば通る** ── `<base>` があれば hash router は無改造で動く。
  path モードは動かないが、それは `allow-same-origin` を足しても同じ
  (document URL が `about:srcdoc` である以上 path は持てない)= **隔離の代償ではない**
- ⚠ **§1-8 の「IndexedDB は救えない」は言い過ぎ**の可能性がある。実装後の検証で
  「opaque origin のまま無改変の IDB アプリを完走させた」という報告があった
  (未確認)。ただし非同期 + カーソル + トランザクション + version change を
  偽装する量は localStorage の比ではないので、**この段では手を出さない**。
  マニュアルには「IndexedDB を使うアプリは動かない」と正直に書いてある
