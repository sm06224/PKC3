...
describe('#582 R1 ── 受け手の引数の仕分け', () => {
  ...
  it('🔴 件数を等値で pin する(推薦の根拠そのもの)', () => {
    ...
    // ⚠ 2026-09-13(#856 段②): `adopt-link-icon` で **P2 が 1 増えた** ──
    //   押した器から飛び先を読むが、それは**いま選んでいるノートの値**である
    //   (`selectedLid` で引ける)。⚠ 既定の `N` に落とすと、doc の主張
    //   (「名前だけで呼べる」の数)が**静かにずれる**
    expect(counts()).toEqual({ P1: 52, P2: 34, E: 23, V: 10, N: 118 });
  });
