...
test('🔴 フォルダに入れる → 読み込み直しても中に居る', async ({ page }) => {
   ...
   *
   * 🔴 **待たずに読み直す**(#258 を直したので)。直す前は作成が「行を書く →
   * ack → 辺を書く」の **2 手**で、ここに ack を待つ 1 行を入れないと落ちた ──
   * つまり**実 user も「フォルダに作った直後に閉じる」と居場所が飛んだ**。
   * ⚠ 待ちを戻すと、この test は**窓が開いたままでも緑**になる(待ちが救う)。
   *   だから待たない ── ここが #258 の回帰を捕まえる唯一の場所である。
   */
  await gotoApp(page);
  await clickReal(page, '[data-pkc-region="browse-tabs"] [data-pkc-browse="filer"]');
  // ルートには**入れたノートが居ない**
