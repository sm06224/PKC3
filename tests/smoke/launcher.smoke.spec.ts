...
test('🔴 取り込んだタイルが同じ順で見えて、押すと開く', async ({ page, context, baseURL }) => {
  ...
  /**
   * 🔴 **opener も referrer も渡さない**(マニュアル「ランチャー」 の約束)を**実物で**見る。
   *
 ...
 *
 * 🔴 マニュアル「本文に書けるもの」 は `asset:<key>` を「本文に書ける形式」として説明していたのに、
...
test('🔴 リンクを足して、リンク先の印を取り込む (#856 段②)', async ({ page }) => {
  ...
  await expect(adopt, '終わったのに「取りに行っています…」のまま').toHaveText('リンク先のアイコンを取り込む');
