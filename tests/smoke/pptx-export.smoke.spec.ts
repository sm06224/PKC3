...
test('🔴 情報ペインの PowerPoint で .pptx が落ちてきて、見出しでスライドが切れる(#187 段⑤)', async ({
  ...
  await page.fill(
    ...
    [
      ...
      '',
      /**
       * 🔑 **接続点を手で書く**(#530 段③b)── 横に並んだ 2 枚なので、
       *   自動なら **右(3)→ 左(1)** になる。**上(0)→ 下(2)** と書いてあるので、
       *   下の検査は「手で書いた辺が配った先まで届いたか」を見分けられる。
       */
      ':::format{.pkc-line from=今日:top to=明日:bottom}',
      '線の中の見えない字',
      ':::',
    ].join('\n'),
  );
  ...
  /**
   * 🔴 **手で書いた辺が、配った先まで届いている**(#530 段③b)。
   * ⚠ ここは**画面と書き出しが合意しているか**を見る唯一の所である ──
   *   本文を読む側(`html-blocks`)と書く側(`pptx`)の unit は、どちらも
   *   相手を模した入れ物と話しているので、綴りの食い違いを見られない(§7)。
   * 🔑 **上(0)→ 下(2)** は、自動で選ばれる **右(3)→ 左(1)** と別物である。
   */
  expect(
    /<a:stCxn id="\d+" idx="0"\/><a:endCxn id="\d+" idx="2"\/>/.test(text),
    '手で書いた辺(上 → 下)が配った先に届いていない',
  ).toBe(true);
