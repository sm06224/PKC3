...
describe('office-wasm のパッチ', () => {
  ...
  /**
   * 🔴 **焼きの既定が、未リリースの alpha に戻っていないか**(2026-09-12)。
   *
   * ⚠ **実際に 14 日間そうなっていた** ── user 裁定 2026-08-29
   *   (「**LibreOffice は配る**…メジャー追従は既に我々のワーク」)で **26.8 を配ると決め、
   *   #511 を閉じた**のに、`lo_ref` の既定は `master`(= 27.2 alpha)のままだった。
   * 🔴 帰結は**静かな逆戻り**である:何も渡さずに焼くと alpha が建ち、
   *   それが `lo-wasm-dev`(**tag 使い回し**)を上書きし、office-pack を回した日に
   *   **user が未リリースの alpha を受け取る**。⚠ 2026-08-30 の 2 本は実際に
   *   この既定で焼かれている(配られてはいないので実害は出ていない)。
   * 🔑 だから**既定を機械で留める** ── 注記に「気をつける」と書くだけでは、
   *   次に触る人は字面が一致しないので気づけない
   *   (2026-09-09 に `pages.yml` で踏んだのと同じ型)。
   * ⚠ **alpha を禁じてはいない** ── `master` と**明示して渡す**のは自由である。
   *   ここが見るのは「**渡さなかったとき何が建つか**」だけ。
   */
  it('🔴 `lo_ref` の既定が未リリースの alpha ではない', () => {
    const yml = readFileSync(YML, 'utf-8');
    const m = /lo_ref:[\s\S]{0,900}?default: '([^']+)'/.exec(yml);
    // ⚠ 空振り防止 ── 入力が消えた / 字が変わったなら、下の比較は何も見ていない
    expect(m, '`lo_ref` の既定を引けない(入力の形が変わった?)').not.toBeNull();
    const ref = m![1]!;
    expect(ref, '既定が master(未リリースの alpha)に戻っている').not.toBe('master');
    // 🔑 **安定メジャーの枝の形**だけ通す(`libreoffice-NN-N`)
    expect(ref, `既定が安定メジャーの枝の形でない: ${ref}`).toMatch(/^libreoffice-\d+-\d+$/);
  });
