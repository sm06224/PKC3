...
describe('閉じを失った本文への書き込み(#318)', () => {
   ...
   *
   * 自前の総当たり(12 種 × 3 行 × 末尾改行 2 通り = **3456 形**)で
   ...
   *
   * 総当たり(12 種 × 3 行 × 末尾改行 2 通り = **3456 形**)で
   * **落ちた行 0 件**を確認した。ここではその不変量を、代表形で常時 pin する。
   */
  it('🔴 修理は挿入だけ ── 原文の行が 1 行も消えない', () => {
    const PIECES = [
      'tags: [あ]',
      // 🔴 井桁で書いた形も回す(#637)── この形は `stripTrailingComment` を
      //    素通りするので、走の長さと修理の位置が別の枝を通る
      'tags: #買い物 #家事',
      'title: メモ',
      'tags:',
      '- あ',
...
/**
 * 🔴 **井桁で書いた `tags:` の次に箇条書きが来たら、箇条書きが値である**
 * (#637 の着地前レビュー D)。
 *
 * ⚠ 1 稿目は「`tags:` の値が井桁の並びなら注釈として刈らない」とだけ書いたので、
 *   次の形で **牛乳と卵が閉じの内側に取り残され、誰も読まなくなって**いた:
 *
 * ```
 * ---
 * tags: #買い物メモ
 ...
 * ```
 *
 * 🔑 守る不変量は既存のもの ── **「隠した行は、必ず誰かが読んでいる」**。
 *   井桁の並びは注釈としても読めるが、**箇条書きは値としてしか読めない**ので、
 *   読める側へ倒す。
 */
describe('井桁の tags: の後ろに箇条書きが来る形(#637)', () => {
  const SRC = '---\ntags: #買い物メモ\n- 牛乳\n- 卵\n---\n本文\n';

  it('🔴 箇条書きのほうがタグになる(行が誰にも読まれなくならない)', () => {
    expect(parseFrontmatter(SRC).meta['tags']).toEqual(['牛乳', '卵']);
  });

  it('🔴 本文は無傷', () => {
    expect(parseFrontmatter(SRC).body).toBe('本文\n');
  });

  /** ⚠ 対照群: 箇条書きが続かなければ、井桁の並びがタグである。 */
  it('⚠ 対照群: 箇条書きが無ければ井桁の並びが値', () => {
    expect(parseFrontmatter('---\ntags: #買い物 #家事\n---\n本文\n').meta['tags']).toBe(
      '#買い物 #家事',
    );
  });
