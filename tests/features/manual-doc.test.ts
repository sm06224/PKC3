...
describe('マニュアルの窓 — 節の印は見出しの字から(D4)', () => {
  ...
  it('🔴 実物のマニュアルで:印は全部一意で、通し番号へ落ちた見出しが無い', () => {
    ...
    /**
     * ⚠ **前提の形が変わった**(2026-09-08、#793 条 6 を満たしたため)。
     *
     * ⚠ 直す前はここで「**重複の `-2` が実際に働いている**」(実測 2 組)を見ていた ──
     *   本文に同名の見出しが在ることが前提だった。
     * 🔴 **その前提は、いま成り立たない** ── 執筆規約 条 6(同じ名前の見出しを 2 つ
     *   作らない)を満たし、`tests/docs-parity.test.ts` が **0 件**を機械で守っている。
     *   ⚠ つまり 2 つの門が**正面から衝突する**:こちらは「同名が 1 組以上在れ」、
     *   あちらは「同名は 0 件であれ」。
     * 🔑 **譲るのはこちら**である ── 重複の解決そのものは、すぐ上の
     *   「🔴 同じ字の見出しが 2 つ在れば 2 つ目は `-2`」が**自前の原稿**で見ている
     *   (しかも `お知らせ` / `そのほか` という、まさにこの本文から消した名前で)。
     *   ⚠ 本文に当てる側で見るのは「**印が全部一意で、通し番号へ落ちていない**」だけでよい。
     * ⚠ だから空振り防止は**その主張に合う形**へ置き換える ── 印を 1 つも作れて
     *   いなければ、上の 2 つの assert は何も言わない(CLAUDE.md §1)。
     */
    expect(ids.length, '実物のマニュアルから印を 1 つも作れていない(空振り)').toBeGreaterThan(100);
  });
});
