...
describe('🔴 足さないとき ── 必ず理由を言う(無言の dead click を作らない)', () => {
  /**
   * ⚠ **2026-09-16 に半分だけ裏返した**(#918 段⑤d-1)。
   * 🔑 **列**は今までどおり断る(1 つの列だけでは、どの表から取り出すか決まらない)。
   * 🔴 **線は組む** ── 自分でキーを繋ぐ道ができた後は、押した 2 つで
   *   `select * from A join B on …` まで決まる(下の it)。
   */
  it('空の欄で列を押したら、先に表を押すよう言う', () => {
    expect(why('', column('売上', '金額'))).toContain('表の名前');
  });

  it('🔴 空の欄で線を押したら、両方の表から組む(断らない)', () => {
    expect(sql('', linkAct())).toBe('select * from 売上\n  join 客 on 客.id = 売上.客id');
    // 🔑 空白だけの欄も「空」として扱う(見た目が空なのに断られるのは読めない)
