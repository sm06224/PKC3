 ...
 *   7. **数式(`$…$` / `$$…$$`)を描くようにした**(2026-09-06、#707。user 裁定
 *      「入れる」)。⚠ **PKC2 でも画面には数式が出ていなかった** ── PKC2 の
 *      `decompose-pkc.ts` は「markdown-it は `$` を素通しするため decompose で拾う
 *      必要あり」と書いており、拾っていたのは **docx の書き出し専用**である。
 *      つまりこれは「PKC2 と違えた」のではなく、**PKC2 が持っていた記法を
 *      画面まで届けた**(user 指示 2026-07-30「流用 + 総合的見直し」)。
 *      🔴 **門を 3 つ置いた**(`math-delims.ts`)── PKC の差し込みの記法
 *      (`${date}` / `${宛名}`)と金額(`$100`)を飲まないため。⚠ とくに
 *      **`${` は開きだけでなく閉じも塞ぐ**:塞がないと
 *      `残りは $x です。${宛名} 様。` の「x です。」が数式になる(実装の 1 稿目が
 *      そうなっており、実測 2026-09-06 で判明)。
 *      動いた golden は `manual-ch12` の **1 行だけ**(段落の途中に書かれた
 *      `$$x^2 + y^2 = z^2$$`)── ⚠ 同じ章の他の `$` **56 個は 1 つも動いていない**
 *      (コードの囲みの中なので markdown-it が先に code として tokenize する)。
 *      🔑 この「動かなかった 56 個」が、**前処理ではなく inline rule で足した**
 *      ことの実測の裏取りである。
 *
