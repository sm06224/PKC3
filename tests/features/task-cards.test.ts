...
describe('task-cards(札 = 本文のチェック項目)', () => {
  ...
  describe('1 件のノートの札だけを差し替える(押した札を往復なしで動かす)', () => {
    ...
    /**
     * 🔴 **盤面に居ないノートの札も足す**(#499、2026-08-28 に**向きを裏返した**)。
     *
     * ⚠ ここは直す前「**入れない**(どこへ入れるべきかは worker しか知らない)」を
     *   pin していた ── つまり **test が欠陥のほうを守っていた**。
     *   実ブラウザで測ると、予定の面から「足す」を押しても本文には書かれるのに
     *   **面には 3 秒経っても出てこない**(user 指摘「意味不明」の実体)。
     * 🔑 「次に面を開けば出る」は、**面を開いたままの user** には届かない理屈である。
     */
    it('🔴 盤面に居ない lid の札も、末尾に足される', () => {
      const next = replaceTaskCards(board, 'zzz', '- [ ] 新しい予定\n');
      expect(next.map(taskCardKey), '新しいノートの札が入っていない').toEqual([
        'a 0',
        'a 2',
        'b 0',
        'zzz 0',
      ]);
      // ⚠ **対照群** ── 既に居るノートの札は 1 枚も動いていない
      expect(next.slice(0, 3)).toEqual(board);
    });

    /**
     * 🔑 **足す物が無ければ同じ配列**(描画側の指紋を無駄に壊さない)。
     * ⚠ 上の向きを裏返したので、**この保証だけは別に置き直す** ──
     *   「入れない」を消したときに、一緒に消えてはいけない性質である。
     */
    it('🔴 チェック項目が 1 つも無い本文なら、配列の同一性を保つ', () => {
      expect(replaceTaskCards(board, 'zzz', 'ただの本文\n')).toBe(board);
    });
