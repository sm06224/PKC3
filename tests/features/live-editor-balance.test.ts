...
const CASES: readonly Case[] = [
  ...
  // ── 数式の塊(2026-09-06、#707)────────────────────────────
  //    🔴 `$$` は **block token** で入れてある(行を畳む前処理にしない)。
  //    ⚠ `source-blocks.ts` は別の走査器で、知っている囲いは fence と `:::` の
  //      2 つだけ ── `$$` を前処理で 1 行に畳むと、ここが `ok: false` になり
  //      **行ごとの編集が無音で全文入力欄へ落ちる**(数式は正しく出るので気づけない)。
  //    ⚠ 直す前は**この経路を守る test が 1 件も無かった**(着地前レビュー M2)──
  //      `SOURCE_LINE_TOKEN_TYPES` から `pkc_math_block` を外す変異が、
  //      unit を全部緑のまま通り抜けていた。
  { name: '$$ の塊', body: '$$\nx^2\n$$\n', ok: true },
  { name: '$$ の塊 + 前後の段落', body: '文\n\n$$\nx^2\n$$\n\n後\n', ok: true },
  { name: '$$ の塊(1 行の形)', body: '前\n\n$$ a^2 + b^2 $$\n\n後\n', ok: true },
