...
describe('② 分割は全域で、重複しない', () => {
   ...
   *
   * renderer が畳まない本文では、走査だけが 1 塊に畳んで食い違う。
   * ⚠ 食い違ったら**開かない** ── 壊れた分割の上で行を差し替えるより安全側である。
   *
   * ⚠ **2026-08-07 に fixture を替えた。** かつては `:::unknown-thing`(知らない名前)を
   * 使っていたが、**走査器が `directive-open.ts` の判定を引くようになった**ので、
   * 知らない名前はもう囲いと見なさない = **開くようになった**(user の動線が戻った)。
   * いま残っている食い違いは「**名前は知っているが、属性が不正で畳めない**」形である。
   * 🔑 かつてここには「走査側にも名前の表を持たせるな(判定が 2 か所になる)」と
   * 書いてあった。その懸念は正しく、だから**表ではなく判定そのものを共有**した ──
   * 表は `directive-open.ts` に 1 つだけ在る。
   */
  it('🔴 描画が畳まない `:::`(不正な id)では開かない側に倒れる', () => {
    const body = ':::figure{id="あ い"}\n\n本文\n\n:::\n\nあと\n';
    // 前提: renderer は畳んでいない(literal のまま出ている)
    expect(renderMarkdown(body, { silentHallucinationWarnings: true })).toContain(
      // ⚠ 引用符は typographer が曲げる(&quot; ではない)ので、そこは見ない
      '<p>:::figure{id=',
    );
    const { part } = build(body);
