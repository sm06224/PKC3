...
describe('markdown の表の升を打つ(#708 段④)', () => {
   ...
   *
   * ⚠ かつてここは「**形を作り変える口(#708 段②)とは判定が違う** ── あちらは
   *   板の中を外している」と書いていたが、**2026-09-07 に揃った**(#743。user 裁定
   *   「出す」)── 外していた理由(作り変えると戻す口が出ない**片道**になる)は
   *   #747 / #775 で消えたためである。
   * 🔑 いまはどちらも**同じ 1 本**(`mdTableRun`)を通る ── 門を足すならそこに足す。
   */
  it('🔴 ::: の板の中の表も、押して打てる', () => {
    const body = ':::note\n\n| 品名 | 数 |\n|---|---|\n| りんご | 3 |\n\n:::\n';
    const m = marks(body);
...
describe('#747 焼いた升と、書ける升を揃える', () => {
  ...
  const CORPUS: readonly { name: string; body: string }[] = [
    ...
    /**
     * 🔴 **#775 で足した 10 形** ── **csv の囲み**が引用の中に在る形と、
     *   **閉じが来ていない**形。⚠ どちらも直す前は
     *   「押せる → 打てる → 消える」だった(実測 2026-09-07:印 4 / 書けた 0)。
     * ⚠ この corpus に csv の囲みが 2 形しか無かったので、#747 の不変量は
     *   **markdown の表しか守っていなかった**(CLAUDE.md §1「corpus に無い形は
     *   通っている test が何も守っていない」)。
     */
    { name: '引用の中の csv', body: '> ```csv\n> a,b\n> 1,2\n> ```\n' },
    { name: '引用の中の csv(深さ 2)', body: '>> ```csv\n>> a,b\n>> ```\n' },
    // 🔑 読み手は**1 段だけ**剥がすので、升の字は `> a`(`>` が 1 つ残る)
    { name: '引用の中の csv(途中で深くなる)', body: '> ```csv\n>> a,b\n> ```\n' },
    // 🔴 閉じが来ていない ── 中身は**引用が終わる所まで**(`end - 1` だと最後の行が落ちる)
    { name: '引用の中の閉じない csv', body: '> ```csv\n> a,b\n' },
    // ⚠ `>` の後ろに空白が無い形(前置きは 1 字)
    { name: '引用の中の csv(素の >)', body: '>```csv\n>a,b\n>```\n' },
    { name: '板の中の引用の csv', body: ':::note\n> ```csv\n> a,b\n> ```\n:::\n' },
    { name: '引用の中の板の csv', body: '> :::note\n> ```csv\n> a,b\n> ```\n> :::\n' },
    /**
     * 🔴 **前置きの無い行で引用が切れる** ── 読み手はそこまでを囲みの中身にする
     *   (実測:表は `a,b` の 1 行だけ。`x,y` は入らない)。
     * ⚠ 閉じの `> ``` ` は**引用の外**に取り残されるので、囲みは**閉じていない**。
     *   だから中身の終わりは `end - 1` ではなく `end` である(#775)。
     */
    { name: '引用が途中で切れる csv', body: '> ```csv\n> a,b\nx,y\n> ```\n' },
    // 🔴 **開きの次の行から前置きが無い** ── 中身が 1 行も無いので表にならない
    { name: '引用の中身が前置きを持たない csv', body: '> ```csv\na,b\n> ```\n' },
    /**
     * 🔴 **素の csv の中の `> `** ── これは**升の字**である(読み手も `> a` を渡す)。
     * ⚠ 行ごとに前置きを数えると、ここを引用と読んで**升の字を食う**。
     */
    { name: '素の csv に > で始まる行', body: '```csv\n> a,b\n1,2\n```\n' },
    // 🔴 **末尾に改行が無い** ── 空行が「閉じの代わり」にならないので最後の行が落ちる
    { name: '閉じない csv(末尾に改行なし)', body: '```csv\na,b' },
    /**
     * 🔴 **深さが食い違う囲みの中の表**(#775 で pin した)。
     *
     * ⚠ 読み手はここを**表として描く**(囲みが閉じないのでコードにならない)。
     *   それでも印を焼かないのは `gateQuotedFences`(前置きを全段剥がした写し)が
     *   外しているからで、**この 2 行だけがその門を守っている** ── 直す前は
     *   門を消しても全 8,180 件が緑のままだった(実測 2026-09-07)。
     * 🔑 押させるかどうかは**見え方の 1 問**なので、#743 とまとめて裁定を仰ぐ。
     *   開けるときは、ここの 0 を 4 に書き換えるのが手順である。
     */
    { name: '深さが食い違う囲みの中の表', body: '>> ```txt\n> | a | b |\n> |---|---|\n> | 1 | 2 |\n>> ```\n' },
    { name: '引用の囲みに前置き無しの表', body: '> ```txt\n| a | b |\n|---|---|\n| 1 | 2 |\n> ```\n' },
    /**
     * 🔴 **#780 で足した 4 形** ── **原文に無い升**(読み手が詰め物で並べる升)。
     *
     * ⚠ この corpus に**升がそろった csv しか無かった**ので、#747 の不変量は
     *   この形を 1 度も見ていなかった ── 実測 2026-09-07:9 升のうち 2 升が
     *   「押せる → 打てる → 消える」だった(CLAUDE.md §1「corpus に無い形は
     *   通っている test が何も守っていない」)。
     */
    { name: '升が足りない行の csv', body: '```csv\na,b,c\n1\n2,3,4\n```\n' },
    { name: '空行のある csv', body: '```csv\na,b\n\n1,2\n```\n' },
    { name: '見出しより升が多い行の csv', body: '```csv\na,b\n1,2,3\n```\n' },
    { name: '引用の中で升が足りない csv', body: '> ```csv\n> a,b,c\n> 1\n> ```\n' },
  ];
  ...
  it('⚠ 印が焼かれる形と、焼かれない形を、corpus の全行で数える', () => {
    ...
    const WANT: readonly [string, number][] = [
      ...
      ['板の中の引用の csv', 2],
      ['引用の中の板の csv', 2],
      // 🔑 切れる**手前まで**が中身(2 升)── 巻き込めば 4 になる
      ['引用が途中で切れる csv', 2],
      // 🔴 **0 が正しい** ── 中身が 1 行も無いので表が描かれない
      ['引用の中身が前置きを持たない csv', 0],
      ['素の csv に > で始まる行', 4],
      ['閉じない csv(末尾に改行なし)', 2],
      // 🔴 **0** ── 読み手は表として描くが、いまは押させない(上の註記。#743 と同じ 1 問)
      ['深さが食い違う囲みの中の表', 0],
      ['引用の囲みに前置き無しの表', 0],
      // ── #780 で足した 4 形(数は**実物の描画**から採った ── 詰め物の升も焼かれる)
      ['升が足りない行の csv', 9],
      ['空行のある csv', 6],
      ['見出しより升が多い行の csv', 6],
      ['引用の中で升が足りない csv', 6],
