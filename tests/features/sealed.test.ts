...
describe('封印 ── 畳んであるが、壊してはいない', () => {
  ...
  it('🔴 2 ペインは組み込みタイルから開ける', async () => {
     ...
     */
    /**
     * ⚠ **`schedule` は引っ越した面ではない**(#673 段②、user 裁定 2026-09-04
     *   「アプリの基本は別窓」)── 左の「予定」タブを**残したまま**、同じ面を
     *   別窓で開く 2 つ目の入口である。ここで見張るのは `calendar` / `kanban`
     *   (引っ越した綴り)が**戻っていないこと**。
     */
    expect(tiles.map((t) => t.kind), '引っ越した面がタイルに残っている').toEqual([
      ...
      'contacts',
      // ⚠ 探す(#680)── 引っ越した面ではなく、新しい組み込み(左に同じ面は無い)
      'search',
      'manual',
      // ⚠ 自分のパソコンで動かす(#532 段 B)── **面ではない**(押すと zip が落ちるだけ)
      'selfhost',
      // ⚠ SQL で調べる(#681 段②)── 新しい組み込み(左に同じ面は無い・別窓で開く)
      'sql',
      // ⚠ 録ったもの(#683 段①)── 連絡先と同じ 2 つ目の入口(左のタブは残す)
      'captures',
    ]);
  });
