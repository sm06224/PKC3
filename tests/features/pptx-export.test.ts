...
describe('🔴 板どうしの線が PowerPoint でも繋がる(#530 段③e)', () => {
  ...
  /**
   * 🔴 **同じ名前の板が 2 枚あっても、線は 1 本に決まる**(#530 段③e)。
   *
   * ⚠ **コピペで必ず起きる** ── 板を複製すると `#名前` もそのまま複製される。
   *   そのとき「どちらを指すか」が決まっていないと、**本文を 1 文字も変えていないのに
   *   書き出すたびに線の行き先が変わる**(再現しない壊れ方である)。
   * 🔑 決めは「**先に書いたほう**」── 本文の順が正本なので、上から読んだ最初の 1 枚。
   * ⚠ 変異試験 M8 が SURVIVED で教えた ── 同じ名前を 2 枚置く fixture が
   *   **1 つも無かった**ので、重複の門が偽になる場面が存在しなかった(CLAUDE.md §1)。
   */
  it('🔴 同じ名前の板が 2 枚あるとき、線は先に書いたほうを指す', () => {
    const r = buildPptx(
      [
        place(0, 0, 200, 100, 1, 'rect', 'a'), p('先'),
        place(0, 300, 200, 100, 1, 'rect', 'a'), p('後(同名)'),
        place(400, 0, 200, 100, 1, 'rect', 'c'), p('相手'),
        pline('a', 'c'),
      ],
      { title: 'T' },
    );
    const xml = partOf(r, 'ppt/slides/slide1.xml');
    const ids = shapeIds(xml);
    // ⚠ 空振り防止 ── 板が 3 枚出ていなければ、下の「先頭を指す」は何も見ていない
    expect(ids.length, '板が 3 枚出ていない').toBe(3);
    const cs = cxns(xml);
    expect(cs, '線が 1 本に決まっていない').toHaveLength(1);
    expect(cs[0]!.st[0], '後から書いた同名の板を指している(上書きされている)').toBe(ids[0]);
    // ⚠ 対照群 ── 2 枚目(同名)は指されていない(「どちらでもよい」実装と区別する)
    expect(cs[0]!.st[0], '2 枚目を指している').not.toBe(ids[1]);
  });
