...
describe('掴んで落とす(同じ群)', () => {
  ...
  /**
   * 🔴 **番号を持たない群で、動かさずに落としても 1 件も書かない**
   * (2026-09-13 の変異試験が教えた ── fixture のゼロ件次元)。
   *
   * ⚠ 上の「並びが変わらない回」の fixture は `order` が **0,1,2 と連番**なので、
   *   「値が変わる行だけ書く」の skip だけで空になっていた ── つまり
   *   **早期 return を外しても落ちなかった**。
   * 🔑 `order` が**全部未設定**だと skip が 1 件も効かないので、早期 return が
   *   無ければ**触っていない 3 件に連番が書かれる**。ここで初めて門が効く。
   */
  it('🔴 番号を持たない群で、動かさずに落としても 1 件も書かない', () => {
    const raw = [t('a'), t('b'), t('c')];
    expect(
      planTileMove(raw, 'b', { kind: 'edge', group: '', anchor: 'c', edge: 'before' }),
      '動かしていないのに、群の全件へ番号を書いた',
    ).toEqual([]);
    // ⚠ 対照群 ── 本当に動かす回は、ちゃんと全件に番号が付く(下の it と対)
    expect(
      planTileMove(raw, 'c', { kind: 'edge', group: '', anchor: 'a', edge: 'before' }).length,
    ).toBe(3);
  });
