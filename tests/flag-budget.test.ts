 ...
 * ## 何を pin するのか
 *
 * ⚠ **書いた当時、PKC3 に flag は 0 個だった**(いまは P11 で 3 個ある)。
 * 数えるものが 0 件の検査は**空振り**であり、それを合格と読むのが
 * この repo が繰り返し踏んだ罠である(`CLAUDE.md` 検証の規律)。
 * だから pin するのは**件数ではなく作法**にした ── 0 個でも 15 個でも同じように
 * 効く形である(下の「検出器の検品」がそれを保証する):
 *
...
/**
 * flag の登記所。⚠ **書いた当時はまだ存在せず**、この test が名前だけ予約していた ──
 * P11 で実際に建った(いま 3 個が宣言されている)。本 test の 3 が
 * 「登記所の外から生やすな」を強制し続ける。
 */
 ...
 * 🔴 **検出器の検品**(空振り防止)。
 *
 * 上の 4 件は、検出器が**何も見つけられない実装でも緑**になりうる
 * (書いた当時は flag が 0 個で、いまも「違反が 0 件」の状態が正常だからである)。
