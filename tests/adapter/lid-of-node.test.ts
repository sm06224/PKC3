...
describe('押した所から lid を引く(#281 検算)', () => {
  ...
  it('🔴 openBody へ落ちる lid の引き方は、必ず lidOfNode を通る', () => {
    ...
    // 🔴 2026-09-06(#708 裁定②、着地前レビュー・実装 R1): ▾ の小窓に「本文を書き換える」行を
    //    出すかどうかも、ここから引く → 6 → 7。⚠ 直す前は `openBody`(**主の枠**)の本文で
    //    行番号を読んでいたので、**留めた枠**の表で押すと**主の枠のノート**が書き換わった
    //    ── 押した物と壊れた物が別、という同じ罠の 3 度目である。
    //    ⚠ **同じ handler の 10 行下**(`noteTitle` = 上の 09-05 の直し)は既に通っていた ──
    //    隣に足した口だけが外れる、が実際の起き方だった
    expect(src.split('lidOfNode(').length - 1, '本文へ書く 7 か所が寄っていない').toBe(7);
    expect(
