...
describe('種別の呼び名', () => {
  ...
  it('⚠ マニュアルが使う呼び名と一致する(「シート」は使わない)', () => {
    ...
    /**
     * ⚠ **製品名も引っかかる**(2026-09-05、#708 段③ を書いていて踏んだ)──
     *   「Google スプレッドシート」と書いた瞬間にここが落ちた。
     * 🔑 **検査は緩めない**(範囲を広く取るのは、旧い呼び名を二度と戻さないため)。
     *   代わりにマニュアルの字を「Google の表計算」にしてある ── 誤って落ちる代償
     *   (2 分)より、旧い呼び名が黙って戻る代償のほうが大きい。
     */
