...
describe('掴んで落とす(#240 段④)', () => {
  ...
  it('🔴 断られた分は件数に入れない(知らせと断りが食い違わない)', async () => {
    /**
     * 🔴 **知らせは「いまそこに在る数」を言う**(2026-08-18 の着地前レビュー M-3)。
     * ⚠ 直す前は**断りが混じる筋を通る test が 1 本も無かった**ので、件数を
     * 「掴んだ数」(`lids.length`)にすり替える変異が生き延びる ── そうなると
     * 画面には「**2 件を「t-f1」へ入れました — ⚠ 1 件は入れられませんでした**」と
     * **自分同士で矛盾する 1 行**が出る。
     * ⚠ フォルダは自分の中へは入れられないので、`f1` と `n1` を `f1` へ落とすと
     *   `f1` だけが断られる(= 掴んだ 2 件・入った 1 件)。
     */
    const { q, d, status } = setup(METAS, []);
    q('tbody [data-pkc-entry="f1"]')!.dispatchEvent(
      dragEvent('drop', dataTransfer({ [PKC_DRAG]: 'f1 n1' })),
    );
    await tick();
    // 空振り防止 ── 断りが実際に起きた筋であること
    expect(d.getState().error ?? '', '断られた 1 件が言われていない').toContain(
      '1 件は入れられませんでした',
    );
    expect(status.at(-1), '断られた分まで「入れました」と数えた').toBe(
      '1 件を「t-f1」へ入れました',
    );
  });
