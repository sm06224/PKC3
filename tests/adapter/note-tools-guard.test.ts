...
describe('ノートが要る道具は、押す前に断る(user 裁定 2026-09-02)', () => {
  ...
  it('🔴 ① ② ③ 4 つとも、断って・何も始めず・押した物の名前を言う', () => {
    for (const { action, label } of NOTE_TOOL_ACTIONS) {
      ...
      /**
       * 🔴 **理由まで pin する**(#666 の着地前レビュー 5)。
       * ⚠ `docs/manual.md` はこの括弧を**逐語で引用**しているのに、検査は
       *   「ノートを開いてから」しか見ておらず、**括弧を消しても緑**だった
       *   (CLAUDE.md §1「文言は『押した場所』と対で pin する」)。
       * 🔑 この理由が真になったのは #666 の裁定からである ── 4 つとも
       *   「開いているノートに入れる道具」になったので、入れ先が決まらない。
       */
      expect(b.error() ?? '', `${label}: なぜ押せないのかを言っていない`).toContain(
        '入れ先のノートが決まりません',
      );
    }
  });
