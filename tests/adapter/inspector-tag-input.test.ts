 ...
 *   **`tags: ["買い物 #家事"]`** と 1 つの名前で書かれていた ── ⚠ **先頭の井桁は
 *   `normalizeTag` が落とす**ので、`#買い物 #家事` という字はどこにも残らない
 *   (2 稿目で実測して訂正。`"#買い物` を grep しても見つからない)。
 *
...
/**
 * 🔴 **2 つ打ったとき、画面が「0 件に付けました」と嘘をつかない**
 * (#637 の着地前の動線レビュー欠陥 1)。
 *
 * ⚠ 1 稿目は打った字を割ったあと **1 つずつ撃って**いた。書込は正しく届くのに、
 *   受ける側は**撃たれた 1 つごとに知らせを 1 通**出し、知らせは**上書き**される ──
 *   だから画面に残る最後の 1 行は **2 つ目のタグの話**だけだった。
 *
 * ⚠ 実測(直す前):`家事` が既に付いているノートに `#買い物 #家事` と打つと
 *   ── disk は `tags: [家事, 買い物]`(**買い物は付いている**)なのに、
 *   画面は **「0 件に付けました / 1 件は既に付いていました」**。
 *   ⚠ フォルダの帯にはタグの札が出ないので、**この 1 行が唯一の手掛かり**である。
 *
 * 🔑 直しは「知らせを 2 通出す」ではなく **1 回の頼みを 1 回で撃つ** ──
 *   `BULK_TAG` が並びを運ぶので、答えも 1 通で全部を語れる。
 */
describe('2 つ打ったときの知らせ(#637)', () => {
  const noticesOf = (s: ReturnType<typeof setup>): string => s.d.getState().notice ?? '';

  it('🔴 付いた側が消えない(名前で言う)', async () => {
    const s = setup('---\ntags: [家事]\n---\n本文\n');
    type(s, '#買い物 #家事');
    await tick(60);
    // 前提 ── 実際に付いていること(付いていないなら文面の話ではない)
    expect(readTags(s.disk['n1']!), '前提が崩れている(そもそも付いていない)').toEqual([
      '家事',
      '買い物',
    ]);
  ...
  /**
   * 🔴 **対照群 ── 1 つのときの字は 1 文字も変えない。**
   * ⚠ 既存の pin(`bulk-tag-wiring.test.ts` / `smart-folder.test.ts`)が
   *   留めている字である。名前を常に付けると、1 つのときの見え方が変わる。
   */
  it('🔴 対照群: 1 つのときはこれまでどおり名前を出さない', async () => {
    const s = setup('本文\n');
    type(s, '買い物');
    await tick(60);
    expect(noticesOf(s)).toBe('1 件に付けました');
  });
});
