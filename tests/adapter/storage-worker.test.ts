...
describe('スマートフォルダの走査(#421 段②)', () => {
  ...
  /**
   * 🔴 **丸ごと読む分岐でも、索引の列を引いている**(2026-08-29 の着地後レビュー)。
   *
   * ⚠ 走査は「チェック項目の条件が在るか」で **SELECT の列を切り替える**。
   *   タグ**だけ**の入れ物は既存 test が通るが、**タグ + 未処理あり**の組み合わせは
   *   1 件も無かった ── だから丸ごと読む側で `body_tags` を落としても
   *   **どの test も落ちなかった**(変異が生き延びる)。
   * ⚠ user から見た症状:「タグ=請求 かつ 未処理あり」の入れ物から、
   *   **本文にしか `#請求` を書いていないノートが黙って全部消える**
   *   (タグだけの入れ物では出るので、理由が読めない)。
   */
  it('🔴 「タグ + 未処理あり」でも、本文の行に書いたタグで集まる', async () => {
    await put(
      'n-both',
      { archetype: 'text', entryOrder: 14 },
      '仕事\n\n- [ ] まだ\n\n#請求\n',
    );
    const got = await scan({ tags: ['請求'], openTasks: true });
    expect(
      got.lids,
      '丸ごと読む分岐で索引の列を引いていない(本文の行のタグが効かない)',
    ).toContain('n-both');
    // ⚠ **対照群** ── タグだけの入れ物では前から出る(この検査が「常に当たる」形でない)
    const tagOnly = await scan({ tags: ['請求'] });
    expect(tagOnly.lids, '前提が崩れている(タグだけでも出ない)').toContain('n-both');
  });
...
describe('本文の csv を SQL から引く(#681 段③)', () => {
   ...
   */
  /**
   * 🔴 **列の目録は「まとめて入れる」ので、区切りを跨いだ形を 1 度は通す**(#968)。
   *
   * ⚠ 直す前は**列の数だけ `exec`** を打っていたので、区切りという概念が無かった。
   *   いまは `CSV_INSERT_CHUNK`(500 行)ずつ積むので、**500 を超えたときだけ
   *   通る道**ができた ── 既存の fixture は列が数個なので、**その道を 1 度も
   *   通っていない**(CLAUDE.md §2「fixture のゼロ件次元は測っていない次元」)。
   * 🔑 だから **1,200 列**(= 区切り 3 回)で、**番号が 0 から抜けも重複もなく並ぶ**
   *   ことを見る ── 区切りの切り出しを間違えると、ここで必ず崩れる。
   */
  it('🔴 列が 1,200 個あっても、目録の番号が抜けも重複もなく並ぶ(#968)', async () => {
    const n = 1200;
    const cols = Array.from({ length: n }, (_, i) => `列${i}`);
    await write('csv-chunk', fence('csv name=広い表', [cols.join(',')]));
    /**
     * ⚠ **ここは `run` を使わない** ── `run` は `maxRows: 100` で切るので、
     *   1,200 列を数えられない(1 稿目はそれで **95** が返り、前提の assert が拾った)。
     * 🔑 切っているのが**計器**か**実装**かを、こういうときに取り違えない。
     */
    const r = await request({
      op: 'runReadOnlySql',
      sql: SCHEMA_CSV_SQL,
    ...
    const wide = r.rows.filter((x) => x[0] === '広い表');
    // ⚠ 前提の assert ── 1 件も返っていなければ、下の一致は「空 = 空」で通る
    expect(wide.length, '広い表 の列が 1 つも返っていない').toBe(n + 2); // _note / _lid が先頭に付く
    // 🔴 番号は 0 から連番(区切りの切り出しを間違えると、ここで崩れる)
    expect(wide.map((x) => x[1]), '番号が抜けたか重複した').toEqual(
      wide.map((_, i) => i),
    );
    // 🔑 区切りの**跨ぎ目そのもの**を名指しで見る(500 / 1000 の前後)
    expect(wide[500]![2], '1 区切り目の終わりで名前がずれた').toBe('列498');
    expect(wide[501]![2], '2 区切り目の頭で名前がずれた').toBe('列499');
    expect(wide[n + 1]![2], '最後の列が落ちた').toBe(`列${n - 1}`);
  });
  ...
  it('🔴 ER が打つ問い合わせが、本文の csv の表と列を返す(#918 段⑤d-2)', async () => {
    ...
    /**
     * 🔴 **行数は「積んだ合計」である**(同じ名前が複数のノートに在る)。
     * ⚠ ここを `> 0` で見ていた 1 稿目は、**`sum()` を外す変異が生き延びる**
     *   ── 集約を外すと sqlite はどれか 1 行(2 か 1)を返すので、どちらも `> 0` を満たす。
     * 🔑 だから**合計そのもの**で留める:`csv-a` が 2 行 / `csv-b` が 1 行 /
     *   `csv-c` が 1 行 ── 積んで **4**。
     */
    expect(Number(sales[0]?.[3]), '行数が「積んだ合計」になっていない').toBe(4);
