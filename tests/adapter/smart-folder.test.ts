...
describe('列で引く条件(#421 段②)', () => {
  ...
  /**
   * 🔴 **外しても本文に残っていることを言う**(2026-08-29 の動線レビューで確定)。
   *
   * ⚠ 外す口(`untag-entry` / 「ここから外す」)が触るのは **frontmatter だけ**なので、
   *   本文の行に `#請求` と書いてあるノートは、外した後も**そのタグを持ったまま**である
   *   (索引は文書タグと本文中タグを合わせて当てる)。
   * ⚠ 黙っていると「外したのに、まだそのタグで集まる」= **壊れて見える**。
   */
  ...
  it('⚠ 対照群: frontmatter にだけ在るタグを外したときは何も足さない', async () => {
    ...
    expect(notice, '外したことを言っていない').toContain('外しました');
  });
