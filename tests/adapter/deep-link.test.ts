...
describe('起動時のディープリンク(#300 段②)', () => {
   ...
   * ⚠ 予算は、11px の字で 1 単位 ≒ 5.5px ── 2 枚目のタブでは前に
   *   「複数タブ: このタブの保存は本体タブ経由です — 」が常設で付くので、
   *   900px 級の窓でもそこまでなら 1 行に収まる、という見立てである。
   *   ⚠ **実測ではない**(撮っていない)ので、はみ出す報告が来たらここを下げる。
   *
   * 🔴 **90 → 110 に上げた**(2026-09-09、#683 段① で 9 面目「録ったもの」が
   *   加わった)。⚠ この行は**面を 1 つ足すたびに 5〜9 単位ずつ伸びる** ──
   *   つまり上げ続けると、いつか本当に切れる。
   * 🔑 **次に足りなくなったら、上げるのではなく畳む**:先頭の数個だけ出し、
   *   残りは「ほか N 個(ヘルプの『開いたときの画面を指定する』に一覧が在ります)」
   *   にする。⚠ ここで上げてよいのは「まだ 1 行に収まる」うちだけである。
   */
  it('⚠ 断り文は 1 行に収まる幅で、打つ字が先に来る', () => {
    const msg = unusableViewMessage();
    const width = [...msg].reduce((n, ch) => n + (ch.codePointAt(0)! > 0x2000 ? 2 : 1), 0);
    expect(width, '状態の行(1 行)に対して広すぎる').toBeLessThanOrEqual(110);
