...
describe('お知らせの文面は固定(#220-7)', () => {
  ...
  const KNOWN: readonly [id: string, digest: string][] = [
    ...
    /**
     * ⚠ **#971 段②(容量の見張り)で足した**(2026-09-16)。
     *   枠(30 件)が満杯だったので、**いちばん古い 1 件**(`2026-09-14-xlsx-sql`)を
     *   落とした ── 原本は `CHANGELOG.md` に在る。
     */
    ['2026-09-16-quota-watch', '72232ed6'],
    /**
     ...
     */
    /**
     * 🔴 **配布済みなのに文面を 1 行直した**(2026-09-18。511f1b6c → e6556745)。
     *
     * ⚠ この entry は「**拾えるだけ取り出す**」というボタン名を案内していたが、
     *   🔴 **その字は 2026-09-16(#986)に画面から消えている**
     *   (口が 2 つに割れて `拾って、戻せる形で書き出す` /
     *   `拾って、読める形で書き出す` になった)。
     * 🔑 だから**届かなくてよい訂正**として直した ── 既読の user には届かないが、
     *   **これから読む人に画面に無い字を探させない**(#996 と同じ型)。
     * ⚠ 訂正の記録は `CHANGELOG.md` の 2026-09-16 の節に置いてある。
     */
    ['2026-09-16-db-rescue', 'e6556745'],
