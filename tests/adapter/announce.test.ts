...
describe('お知らせの文面は固定(#220-7)', () => {
  ...
  const KNOWN: readonly [string, string][] = [
    ...
    /**
     * ⚠ **一度落としたが、着地前に戻した**(着地前レビュー・実装 R2、2026-09-06)。
     * 🔑 落とした理由に「v3.2.0 に入っている」と書いていたが**事実と逆**で、
     *   これは 1 度も配っていない ── 落とせば**誰の画面にも出ないまま消える**。
     * 🔑 枠は `2026-09-05-table-format-convert`(右クリック)を
     *   `2026-09-06-table-convert-touch`(▾)へ**束ねて**空けた(同じ小窓の同じ操作)。
     */
