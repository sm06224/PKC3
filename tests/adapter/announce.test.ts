...
describe('お知らせの文面は固定(#220-7)', () => {
  ...
  const KNOWN: readonly [id: string, digest: string][] = [
    ...
    /**
     * ⚠ **#984(並べ替えの重さ)で足した**(2026-09-16)。枠(30 件)が満杯だったので、
     *   **いちばん古い 1 件**(`2026-09-14-sql-schema-note`)を落とした
     *   ── 原本は `CHANGELOG.md` に在る(配布済み: 576c3d1b)。
     */
    ['2026-09-16-reorder-light', 'db694316'],
    /**
     * ⚠ **#973(窓 2 つで同じ番号)で足した**(2026-09-16)。枠(30 件)が満杯だったので、
     *   **いちばん古い 1 件**(`2026-09-14-voice-boost`)を落とした
     *   ── 原本は `CHANGELOG.md` に在る(配布済み: 0307debf)。
     */
    ['2026-09-16-folder-place-keep', '2a4dcc2e'],
    /**
     * ⚠ **#968(広い csv の重さ)で足した**(2026-09-16)。枠(30 件)が満杯だったので、
     *   **いちばん古い 1 件**(`2026-09-14-audio-trim`)を落とした
     *   ── 原本は `CHANGELOG.md` に在る(配布済み: 000a123b)。
     */
    ['2026-09-16-wide-csv-speed', 'ee77e7b5'],
    /**
     ...
     */
    /**
     * 🔴 **配布済みの文面を直した ── 届かなくてよい訂正**(2026-09-16)。
     *
     * ⚠ この 1 件は `origin/main` の `e735fb64` で**既に配っている**
     *   (`git log --oneline origin/main -S"2026-09-16-big-zip" …` が 1 行返す)ので、
     *   既読の user には**この直しは届かない**。それでも直した理由:
     * 🔴 **「一式を書き出す」というボタンは画面のどこにも無い**
     *   (`commands.ts` の label は「**バックアップ**」)── つまり
     *   **これから読む人を、在りもしないボタン探しへ送る字**だった。
     * 🔑 直すのは「届く」ためではなく「**これ以上、嘘を出さない**」ためである。
     */
    ['2026-09-16-big-zip', '582570e4'],
    /*
     * ⚠ **文面を 1 度直している**(同じ 2026-09-16)。「持ち歩ける 1 枚」の
     *   行を 1 つ足した ── 🟢 **まだ配っていない**ので文面を直してよい。
     * 🔑 確かめ方:`git log --oneline origin/main -S"2026-09-16-big-zip" …` → **0 行**
     *   (⚠ 対照群 `2026-09-16-quota-watch` は a0a846f7 を返す ── 引く先は生きている)。
     */
