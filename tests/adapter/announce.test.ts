...
describe('お知らせの文面は固定(#220-7)', () => {
  ...
  const KNOWN: readonly [string, string][] = [
     ...
     * ⚠ 落とした 1 件は**まだ配っていない**が、原本は CHANGELOG に在る。
     */
    ['2026-09-07-csv-empty-cell', 'edeaa23e'],
    ['2026-09-07-append-scroll', '9eabafe9'],
    ['2026-09-07-inline-calc', 'ad8c3c56'],
    ['2026-09-06-touch-cell-mark', '720f55d7'],
    ['2026-09-06-math', '7ee66d7b'],
    /**
     * ⚠ **配る前に 2 行足した**(2026-09-07、#779。**user 裁定「割り直す(本文はそのまま)」**)──
     *   マニュアルの章立てを組み直したので、同じ主題(マニュアルが読みやすくなった)へ束ねた。
     * 🔑 観測点:`git show v3.2.0:src/features/notice/notice-log.ts | grep '^    id: '`
     *   に `2026-09-06` の id は 1 件も無い(= 誰の画面にも出ていない)。
     * 🔑 **枠を空けずに済ませた** ── item は 4 → 6 で上限ちょうどである。
     */
    ['2026-09-06-help-manual-first', '60667ef7'],
    /**
     * ⚠ **まだ 1 度も配っていないので、字を直して pin し直した**(2026-09-06)。
     * 🔑 観測点は 1 つ:`git show v3.2.0:src/features/notice/notice-log.ts | grep '^    id: '`
     *   が返す id は**全部 2026-08-29** ── 09-05 / 09-06 の entry は
     *   最新の安定 tag に 1 件も入っていない(= 誰の画面にも出ていない)。
     * ⚠ 直した理由は**同じ小窓の話を 3 通が別々に言っていた**こと
     *   (着地前レビュー・動線 D4)── 題名「この表をコピー」はこの PR で
     *   変わり、「もう一度右クリックすれば戻せます」は指の端末では**できない**。
     */
    /**
     * ⚠ **まだ 1 度も配っていないので、pin し直した**(2026-09-06、#722 P2-10)。
     * 🔑 「主の操作を 1 つだけ濃く」の枠を空けるため、`2026-09-05-table-copy-formats`
     *   (持ち出し)を**この 1 件へ束ねた** ── どちらも**表の右上の「▾」の小窓**の話で、
     *   題名も「▾ から、持ち出しも形の書き換えも」へ書き直した。
     * ⚠ 束ねるときに**行を落とさない**(#758 で 1 行落として直した)── 持ち出しの
     *   4 行は 6 件の枠に畳んで全部入れてある(⧉ の手数の話も残した)。
     * ⚠ **それでも 2 行と題名を落としていた**(着地前レビューが全数突合で見つけた)──
     *   「その表だけが作り変わります」「何行目の何列目かを出して断ります」と、
     *   題名の「**(指で触る端末でも)**」。⚠ 最後のは #708 裁定②の**主題そのもの**
     *   (右クリックできない端末にも届く)なので、落ちると裁定の成果が伝わらない。
     */
    /**
     * ⚠ **未配布なので pin し直した**(2026-09-06、着地前レビュー)── 「字が**白く**
     *   反転します」が **9 テーマ中 6 つで逆**だった(暗いテーマは `--fg` が明るいので
     *   **明るい地に暗い字**になる)。「濃くなるのは各画面で 1 つだけ」も PC では嘘
     *   (左と中央で 2 つ)なので、**数えられる字**へ直した。
     */
    /**
     * 🔴 **`main` に在る姿へ戻した**(2026-09-06、着地前レビュー・動線 6)。
     *
     * ⚠ 一度は #720 ②(タブの下線)の 1 行をここへ足したが、**この id は既に
     *   `main` = `/dev/` に出ている** ── カードは `unreadNotices` が **id だけ**で
     *   判定する(中身を見ない)ので、**一度閉じた人には二度と出ない**。
     * ⚠ user 指示 2026-09-02 は「**お知らせに変更を入れるのを忘れずに / user は実機の
     *   お知らせで変更を知る**」である ── 既配布の id への追記は、その約束を満たさない。
     * 🔑 だから**新しい 1 件**(`2026-09-06-help-manual-first`)へ移した。
     */
    /**
     * ⚠ **配る前に 2 行足し、1 行を狭めた**(2026-09-08、#761)── 編集中の
     *   「+ ノート」を**押せない見た目**にし、鍵で撃ったら理由を出すようにした。
     * 🔴 同じ entry の「**押せる場所と押した結果は、これまでと 1 つも変わりません**」が
     *   **嘘になる**ので、そこを外した(この波でまさに押した結果が変わる)。
     * 🔑 観測点:`git show v3.2.0:src/features/notice/notice-log.ts | grep '^    id: '`
     *   に `2026-09-06` の id は 1 件も無い(= 誰の画面にも出ていない)。
     */
    ['2026-09-06-primary-action', 'd57265a9'],
    /**
     * ⚠ **配る前に字を直した**(2026-09-07、#743。**user 裁定「出す」**)──
     *   「::: の中の表も書き換えられません」と書いてあったが、**書き換えられるように
     *   直した**ので嘘になる。⚠ 引用(>)の中も同じなので、1 行足して pin し直した。
     * 🔑 観測点:`git show v3.2.0:src/features/notice/notice-log.ts | grep '^    id: '`
     *   に `2026-09-06` の id は 1 件も無い(= 誰の画面にも出ていない)。
     * 🔑 **枠を空けずに済ませた** ── 同じ話(表の形の書き換え)なので束ねた。
     *   ⚠ 新しい 1 件を足すと、いちばん古い**未配布の** 1 件が押し出される。
     *   ⚠ item も 6 件で満杯だったので、**ほぼ同じことを言っていた 2 件**
     *   (`.csv で保存` と `⧉ の 1 押し`)を 1 件に畳んで枠を作った ──
     *   どちらの事実も落としていない(字は 104 字で上限 120 の内側)。
     */
    ['2026-09-06-table-convert-touch', '049c8d6b'],
    /**
     * ⚠ **まだ配っていないので、#750 I1 の 2 行を足して pin し直した**(2026-09-06)。
     * 🔑 新しい entry を作らなかったのは、枠(10 件)が**全部未配布**で埋まっており、
     *   押し出す相手を「もう配ったもの」から選べないからである(#754 の R2 と同じ事情)。
     *   ⚠ 中身は**同じ主題**(表のセルを押して打つ)なので、束ねるのが素直である。
     */
    // ⚠ まだ配っていない(#722 P2-11 で足した entry)
    ['2026-09-06-reading-column', '4837b56d'],
    /**
     * ⚠ **#722 P2-11 の枠を空けるため、`2026-09-05-md-table-cell` をここへ束ねた**
     *   (2026-09-06)── どちらも未配布で、主題が同じ(表のセルを押して打つ)。
     * 🔑 観測点は #754 と同じ:`git show v3.2.0:…/notice-log.ts | grep '^    id: '`
     *   が返す id は**全部 2026-08-28 / 08-29** ── 09-05 / 09-06 は 1 件も無い。
     */
    /**
     * ⚠ **束ねたときに 1 行落としていたので、pin し直した**(2026-09-06、着地前レビュー)──
     *   `2026-09-05-md-table-cell` に在った「行や列を足す ＋ × は csv の表だけ /
     *   ▾ から書き換えれば使える」は**発見の導線**なので、1 行目へ畳んで戻した
     *   (枠は 6 件で満杯なので item を増やさず、同じ item に入れた)。
     */
    /**
     * ⚠ **配る前に字を直した**(2026-09-07、#749)── 「引用(>)の中の表を
     *   押せないようにした」と書いてあったが、**打てるように直した**ので嘘になる。
     * 🔑 観測点:`git show v3.2.0:src/features/notice/notice-log.ts | grep '^    id: '`
     *   に `2026-09-06` の id は 1 件も無い(= 誰の画面にも出ていない)。
     */
    /**
     * ⚠ **配る前に字を直した**(2026-09-07、#775)── 「引用(>)の中の表も…打てます」と
     *   書いてあったが、**csv の囲みで書いた表は打てなかった**(押せて打てるのに、
     *   確定すると打った字が消えていた)。直したので「表も csv の囲みも」に足し、
     *   直した件数を 5 → 6 にした。
     * 🔑 観測点:`git show v3.2.0:src/features/notice/notice-log.ts | grep '^    id: '`
     *   に `2026-09-06` の id は 1 件も無い(= 誰の画面にも出ていない)。
     * 🔑 **枠を空けずに済ませた** ── 同じ話(表のセルを押して打つ)なので束ねた。
     *   ⚠ 新しい 1 件を足すと、いちばん古い**未配布の** 1 件が押し出される
     *   (すぐ下の `2026-09-05-low-window-append` の注記が、それを戒めている)。
     */
    ['2026-09-06-table-cell-fixes', '96bebe7a'],
