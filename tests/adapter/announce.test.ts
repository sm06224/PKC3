...
describe('お知らせの文面は固定(#220-7)', () => {
   ...
   */
  const KNOWN: readonly [id: string, digest: string, shipped?: 'main'][] = [
    /**
     ...
     */
    ['2026-09-14-place-lines', '25e30596', 'main'],
    /**
     ...
     */
    ['2026-09-14-sql-input-keys', '01dcd516', 'main'],
    /**
     ...
     */
    ['2026-09-13-app-open-target', '1a5f5616', 'main'],
     ...
     */
    ['2026-09-13-app-group-order', '6fed0c31'],
    /**
  ...
  it('🔴 既に配った文面が変わっていない(変えても既読の user には届かない)', () => {
    ...
    const shipped = new Set(KNOWN.filter(([, , ship]) => ship === 'main').map(([id]) => id));
    // 🔑 空振り防止 ── 印が 1 つも付いていなければ、下の分岐は片方しか走らない
    expect(shipped.size, '配布済みの印が 1 つも無い(印を付け忘れている)').toBeGreaterThan(0);
    const drift: string[] = [];
      ...
      if (want !== digest(n))
        drift.push(
          `${n.id}: 文面が変わっている(${want} → ${digest(n)})` +
            (shipped.has(n.id)
              ? // 🔴 配った id は「この表を直す」で済ませられない ── 帯に出ないから
                ' ── 🔴 **この id はもう配ってある**(main に在る)。中身を変えても既読の' +
