...
describe('お知らせの文面は固定(#220-7)', () => {
  ...
  const KNOWN: readonly [string, string][] = [
    ...
    ['2026-08-29-split-open-at', '8454a2af'],
    /**
     * ⚠ **この entry は main に在る = `/dev/` へは配ってある**(`a091456` / #595)。
     *   ⚠ だから「まだ配っていない」とは書けない ── 実測: `git tag --contains a091456`
     *   は**空**で、本番(最新の安定 tag = v3.2.0)には入っていない。
     * 🔑 それでも文面を直したのは、**嘘を配ったままにしないため**である ──
     *   1 稿目は「右クリックで出る項目に…説明が出ます」と書いていたが、この entry が
     *   案内している**見出しの右クリックには説明が付かない**(着地前の動線レビューが
     *   拾った)。⚠ dev の読み手が既に閉じていれば、この訂正は届かない ── それでも
     *   「起きていないことを書かない」(`notice-log.ts` の禁止 3)を優先する。
     */
    // ⚠ 訂正(2026-08-30, #628): 直訳っぽい字を言い直した(c97d95d2 → 2542df94)。挙動は変えていない
    ['2026-08-29-heading-context-menu', '2542df94'],
    // ⚠ 訂正(2026-08-30, #628): 直訳っぽい字を言い直した(e6d0b4b3 → 764736ca)。挙動は変えていない
    ['2026-08-29-board-to-pptx', '764736ca'],
    // ⚠ 訂正(2026-08-30, #628): 直訳っぽい字を言い直した(8f325cfd → 5b8db446)。挙動は変えていない
    ['2026-08-29-narrow-no-hscroll', 'f7b40694'],
    // ⚠ 指紋を差し替えたのは **まだ配っていない entry** だから(着地前に文面を直した)
    // ⚠ 訂正(2026-08-30, #628): 直訳っぽい字を言い直した(07081d45 → 21b0140a)。挙動は変えていない
    ['2026-08-29-reachable-controls', '6729fee7'],
    // ⚠ 訂正(2026-08-30, #628): 直訳っぽい字を言い直した(4f773942 → b8870e11)。挙動は変えていない
    ['2026-08-29-office-26-8', 'b8870e11'],
    // ⚠ 訂正(2026-08-30, #628): 直訳っぽい字を言い直した(6f5b67a5 → 5d0386e9)。挙動は変えていない
    ['2026-08-29-split-frames', '5d0386e9'],
    ['2026-08-29-body-context-columns', '6ca52dbd'],
    ['2026-08-29-deleted-stays-in-lists', 'd2e042ec'],
    // ⚠ 指紋を差し替えたのは **まだ配っていない entry** だから(着地前に文面を直した)
    // ⚠ 訂正(2026-08-30, #628): 直訳っぽい字を言い直した(c93f2f64 → d335638b)。挙動は変えていない
    // ⚠ 上限 10 を超えたので 2026-08-30 に落とした(原本は CHANGELOG)
    // ['2026-08-29-import-drop-undo', 'd335638b'],
    /**
