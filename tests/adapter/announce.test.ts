...
describe('お知らせの文面は固定(#220-7)', () => {
  ...
  const KNOWN: readonly [string, string][] = [
     ...
     */
    ['2026-09-06-help-manual-first', 'cc676806'],
    /**
     ...
     */
    /**
     * ⚠ **#720 ②(選んでいるタブの下線)をここへ束ねた**(2026-09-06)── 枠(10 件)は
     *   まだ全部未配布なので、押し出す相手を「もう配ったもの」から選べない。
     * 🔑 主題が同じ(**どれが「いま」で、どれが「次」かを見て分かるようにする**)ので、
     *   題名を広げて 1 行足した ── **落とした行は無い**。
     * ⚠ この id は `/dev/` には既に出ている ── そこで閉じた人には**この 1 行は
     *   カードとして届かない**(ヘルプの「これまでのお知らせ」では読める)。
     *   本番へはまだ 1 度も出していないので、配る先の user には全文が届く。
     */
    ['2026-09-06-primary-action', '1a2821fc'],
    ...
    ['2026-09-05-scan-after-save', 'a14aa070'],
