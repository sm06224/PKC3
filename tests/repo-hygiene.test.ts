...
describe('🔴 smoke の spec は黙って消えない(2026-08-29)', () => {
  it('🔴 spec の file 数と test の件数が変わっていない', () => {
    ...
    // ⚠ 2026-09-04(#663): `phone` に **+2 件** ── ①横向き 844×390 でもスマホ用画面
    ...
    // ⚠ 2026-09-05(#684 段②): 同じ file に「一覧の行を本文へ落とすとリンクが入る」**+1 件**
    //    (一覧の行が本物の D&D で本文の面まで運ばれ、押せるリンクとして描き直されるのは実ブラウザだけ)
    ...
    // ⚠ 2026-09-05(#579): `context-menu` に「章の参照をコピー → 貼って押すと見出しまで送られる」
    ...
    // ⚠ 2026-09-05(#703): `layout` に **+3 件**(タブレット 768×1024 / 1024×768 で情報が畳んで開き、
    ...
    // ⚠ 2026-09-05(#708 段①): `table-copy.smoke.spec.ts` を**新設**(+1 file / **+1 件**)──
    //    ▾ から形を選んで**本物のクリップボードへ入る**か / 器(`<dialog>`)が本当に開いて
    //    押した行が答えになるか / ⧉ と ▾ が右上で**重なって押せなくなっていない**か。
    //    3 つとも happy-dom には無い層である(座標・`showModal()`・`navigator.clipboard`)
    // ⚠ 2026-09-05(#708 段②): `table-format.smoke.spec.ts` を**新設**(+1 file / **+1 件**)──
    //    ①本物の右クリックでメニューが出るか ②メニューの項目から実際に形が変わるか
    //    ③**読み直しても残るか**(本文へ書かれた証拠)。3 つとも happy-dom には無い層である
    // ⚠ 2026-09-05(#708 段④): `md-table-cell.smoke.spec.ts` を**新設**(+1 file / **+1 件**)──
    //    ①押した升に**本物の焦点**が入るか ②`execCommand` の道が通るか(happy-dom には
    //    無いので unit は必ず fallback を通る)③**読み直しても残るか**(disk まで届いた証拠)。
    //    ⚠ 升に `\|` を入れてある ── 逃がし直さないと**確定した瞬間に列が増える**
    // ⚠ 2026-09-06(#722 P2-10): 主の操作の見え方を見る spec を 1 本足した → 88 → 89
    // ⚠ 2026-09-06(#707): 数式の spec を 1 本足した → 89 → 90。
    //    ⚠ かつてここに「unit では原理的に見えない」と書いたが**嘘だった**
    //    (`setMathWorkerSpawn` で偽ワーカーを差せる ── CLAUDE.md §2)。
    //    ここでしか見えないのは「**本物の KaTeX で組めるか / 書体が当たるか**」だけ
    // ⚠ 2026-09-07(#764): その場で計算の spec を 1 本足した → 90 → 91。
    //    ⚠ **ここでしか見えない層が 1 つ在る** —— 答えを差し込んだ後の改行は
    //    `preventDefault` せずブラウザ既定に委ねているので、happy-dom では走らない
    // ⚠ 2026-09-07(#782): 追記のスクロールの spec を 1 本足した → 91 → 92。
    ...
    // ⚠ 2026-09-09(#532 段 B): 「自分のパソコンで動かす」を押すと一式が落ちる
    //    spec を 1 本足した → 93 → 94。🔴 **ここでしか見えない層が 2 つ**:
    //    ①`<a download>` の実配線(unit は偽の download を差している)
    //    ②配っている物を**配っている場所から相対で**本当に取れるか
    // ⚠ 2026-09-09(#678): コピーした物の履歴を 1 本足した → 94 → 95。
    //    🔴 **ここでしか見えない層が 3 つ**:①本物のクリップボードへ写るか
    //    (unit は navigator.clipboard を差した偽物で見ている)②メニューが出て押せるか
    //    ③localStorage の往復(読み直しても残るか)
    // ⚠ 2026-09-11(#770 段①): 図案が書体から出ていることを見る spec を 1 本足した
    //    → 95 → 96。🔴 **ここでしか見えない**:woff2 は畳まれていて unit から中身を
    //    読めず、happy-dom は書体を持たない ── 「豆腐(□)になっていない」は
    //    実ブラウザでしか言えない(送り幅を素の書体と突き合わせる)
    // ⚠ 2026-09-16(#918 段⑤d-2/d-3): 本文に書いた名前つき csv の表が**つながり図に
    //    四角として出る**ことと、**線が 0 本のときの理由文**を見る spec を 1 本足した
    //    → 96 → 97。🔴 **ここでしか見えない層が 2 つ**:①本文の csv は
    //    **引くたびに temp へ組み立てる**ので、実際に worker が `csv_columns` を
    //    作って ER がそれを読むところは**本物の sqlite でしか通らない**
    //    ②「繋ぐ」を入にすると**理由は残り、誘いだけ消える**という 2 段の字は、
    //    描画と state をまたぐので unit では片側しか見えない
    // ⚠ 2026-09-20(#1017 段⓪): 「システム」の先頭の目次(押すと節へ / 「上へ」で戻る /
    //    hash を壊さない / 改名のお知らせ)を 1 起動で通す spec を 1 本足した → 97 → 98
    // ⚠ 2026-09-21(#1017 段④a): 何も選んでいないときの右の列(コレクションの件数 +
    //    書き出し 4 つ)を 1 起動で通す spec を 1 本足した → 98 → 99
    // ⚠ 2026-09-21(設計 doc §7、段②a): メッセージ(一覧に出ない / 開くと中央に出る /
    //    編集が出ない / hash 不変)を 1 起動で通す spec を新設 → 98 → 99。
    //    🔴 ここでしか見えない層:「開いた面が普通のノートの detail と同じ骨組みで
    //    描かれ、しかし『編集』が DOM に無い」は unit(happy-dom)でも見えるが、
    //    「左の一覧(実 DOM)に絶対混ざらない」ことは実ブラウザでしか言えない
    // ⚠ 段④a と段②a を同じ日に足したので 98 → 100
    // ⚠ 2026-09-21(#1017 段④b): 左下のバックアップと情報ペインの
    //    バックアップ(このノート)が別の末尾(`.pkc3-full.zip` / `.pkc3-notes.zip`)を
    ...
    // ⚠ 2026-09-05(#708 段①、着地前レビュー C-1): 指の端末で右上の 3 つが重ならない
    //    ことを見る 1 本を足した(⧉ と ▾ が 24px になるので ‹/› が 2px 食い込んでいた)→ 456 → 457
    // ⚠ 2026-09-06(#708 裁定②): ▾ の小窓から形を変える 1 本と、
    //    `:::` の中では出さない 1 本を足した → 462 → 464
    // ⚠ 2026-09-06(#745): 3 本足した → 459 → 462。⚠ **同じ tick で撃つ**必要がある
    //    (`locator.click()` は要素が落ち着くのを待つので、待った回はこの不具合を
    //    またいで緑になる ── 1 稿目はそれで直す前でも緑だった)。足したのは
    //    ①升を続けて 2 つ打てる ②Escape で消した字が戻らない
    //    ③**csv の側**(升の口を焼く場所は 2 つ在るので、経路ごとに置く)
    // ⚠ 2026-09-06(#750 I1): `Tab` で右・`Enter` で下へ、押し直さずに続けて打てる
    //    1 本を足した → 464 → 465。⚠ **unit では原理的に見えない** ── 隣を開けるのは
    //    `applyBlocks` が塊を差し替えた**後**で、その経路は実物の描画でしか走らない
    //    (#745 と同じ理由)。観測点は「欄が開いた」ではなく**読み直しても残る字**にした
    // ⚠ 2026-09-06(#722 P2-11): 本文が列の中央に置かれ、**表が段落と同じ左端に揃う**
    //    1 本を足した → 465 → 466。⚠ happy-dom に版面は無いので unit では原理的に見えない
    // ⚠ 2026-09-06(#722 P2-10): 主の操作だけ**地と字が反転して見える** 1 本を足した
    //    → 466 → 467。⚠ unit は印が付いているかしか見られない ── **本当に濃いか**と
    //    **乗せても濃いまま**(`button:hover` のほうが詳細度が高い)はここでしか見えない
      // ⚠ 2026-09-06(#707): 数式の 1 本を足した → 467 → 468。着地前レビューを受けて
      //    さらに 2 本(書き間違えた式 / 書式パネルの「数式」)→ 468 → 470
      // ⚠ 2026-09-06(#750 I2): 指で触る端末の升の印と、**その対照群**
      //    (マウスの端末には出さない)で 2 本 → 470 → 472
      // ⚠ 2026-09-07(#764): 計算する 1 本と、**その対照群**(式でない `=` は
      //    ただ改行する)で 2 本 → 472 → 474。着地前レビューを受けて
      //    **引用の行でも取り消せる** 1 本(#765 をこの PR で直した)→ 474 → 475
      // ⚠ 2026-09-07(#773): 全角で打つと式ごと半角になる 1 本 → 475 → 476。
      //    ⚠ **unit では見えない層が 2 つ** ── 半角へ直す一手と答えを挿す一手が
      //    続けて撃たれても位置がずれないか / 取り消しで打った全角へ戻れるか
      // ⚠ 2026-09-07(#749): 引用の中の升を押して打ち、**`>` が動かない**ことを
      //    開き直して見る 1 本 → 476 → 477(file は増えていない)
      // ⚠ 2026-09-07(#782 A): 別の窓が追記してもこちらが飛ばない 1 本 → 477 → 478
      // ⚠ 2026-09-07(#782 B): 追記した窓は**足した所へ送る** 1 本と、
      //    **対照群 2 本**(①見えているなら動かさない ②空のまま押しても動かない)
      //    → 478 → 481。⚠ ①が無いと「常に一番下へ飛ばす」実装が、
      //    ②が無いと「`lastAppend` が動いた回だけ」の門が、それぞれ素通りする
      //    (実測:どちらも変異が生き延びた)
      // ⚠ 2026-09-07(#780): 原文に無い升を押して打ち、**開き直しても残る** 1 本
      //    → 481 → 482。⚠ unit では押して打つ往復が通せない(欄を開くのも、
      //    確定して本文へ届くのも実 DOM の話である)
    // ⚠ 2026-09-07(#743): 引用と `:::` の中でも形を変えられることを見る 1 本を足した
    //    → 482 → 483。🔴 **ここでしか見えない層が本体である** ── 壊れ方は
    //    「升の字は 1 文字も変わらないのに、表だけ引用から抜け落ちる」なので、
    //    升を数える unit では見えない(見えるのは `blockquote` の中に在るか、である)。
    //    ⚠ 同じ波で `table-copy` の「::: では出さない」の**台を差し替えた**
    //    (裁定で出るようになったため → 閉じていない csv の囲みへ。件数は変わらない)
    // ⚠ 2026-09-08(#761): 編集中の「+ ノート」が薄くなり、鍵で撃つと理由が出ることを
    //    見る 1 本を足した → 483 → 484。🔴 **ここでしか見えない層が 2 つ**:
    //    ①**`disabled` に CSS が本当に効くか**(happy-dom は CSS を組まないので属性しか
    //    見えない)②**本物の鍵**を撃ったときに画面へ理由の 1 行が出るか(unit は
    //    `runGlobalCommand` を直に呼ぶので、鍵の配線を通らない)。
    //    ⚠ 🔴 2026-09-08 に註記を直した ── ここには「主の印を外し忘れると濃いままになる」と
    //    書いてあったが**嘘**だった(薄さは `button:disabled` の 1 本だけが出す。
    //    印を落としても薄さは変わらない)。印そのものは spec の側で直に見ている
      // ⚠ 2026-09-08(#722): 本文の置き場所の smoke を 1 本足した
      // ⚠ 2026-09-08(#750 I2): 指で触る端末への合図の smoke を 1 本 +
      //    「マウスの端末には出さない」対照群を 1 本(変異 S4 を殺すのに要る)
      // ⚠ 2026-09-08(#684 段④): 外から落とした file が**落とした所**へ入る smoke を
      //    1 本足した → 488 → 489。🔴 ここでしか見えない層:実レイアウトの座標で
      //    `DataTransfer(Files)` を受け、bytes を IDB へ置き、本文を書き替えて
      //    **絵として描き直す**往復(unit は「位置が渡る」までしか見ない)
      // ⚠ 2026-09-09(#684 段③): 塊を一覧の行へ落として**別のノートへ移る**まで見る
      //    smoke を 1 本足した → 489 → 490。🔴 ここでしか見えない層:一覧の行が
      //    **落とし先になっているか**(`data-pkc-drop` はフォルダにしか付いていないので、
      //    unit の台を作った時点では気づけず、実ブラウザで初めて「落ちない」と分かった)
      // ⚠ 2026-09-09(#684 ㋑): 横に留めた枠へ file を落とすと**その枠のノート**へ
      //    入るまで見る smoke を 1 本足した → 490 → 491。🔴 ここでしか見えない層:
      //    **本当に 2 枠並んでいる**状態で、留めた枠の座標に線が出て、
      //    **その枠の本文だけ**が描き直され、主の枠が 1 文字も動かないこと
      // ⚠ 2026-09-09(#812): 狭い画面でマニュアルの目次が畳まれる smoke を 1 本
      //    足した → 491 → 492。🔴 ここでしか見えない層:`details` の閉じ方は
      ...
      // ⚠ 2026-09-09(#532 段 B): 一式が落ちて、中に sw.js まで入っていることを見る
      //    1 本 → 493 → 494。⚠ サーバを立てるところまではやらない(固定ポートを掴むと
      //    他の spec とぶつかる)── そちらは手元の probe で通した
      // ⚠ 2026-09-09(#678): コピーした物が残り、選ぶともう一度コピーされる 1 本
      //    → 494 → 495
      // ⚠ 2026-09-11(#770 段①): 図案が書体から出ていることを見る 1 本 → 495 → 496
      // ⚠ 2026-09-12(#855 段 0): **ずっと前から「毎日」**と書いた行が今日の予定に
      //    出ることを見る 1 本 → 496 → 497。🔴 ここでしか見えない層:展開の関数は
      //    unit が直に呼べるが、**関数が返しているのに升目に札が出ない**(窓の切り方と
      //    描画が間に挟まる)は実物でしか見えない。⚠ 既存の「毎週」の道中には
      //    載せられない ── `毎日` を足すと**窓の全部の日に札が増え**、あちらの
      //    数の assert 4 か所が意味を変える(1 test = 1 主張)
      // ⚠ 2026-09-13(#857 段①b-2): タイルを**指で長押し**して並べ替えモードに入り、
      //    押しても窓が開かず、「上へ」で動き、「並べ替えを終える」で抜ける 1 本
      //    → 497 → 498。🔴 ここでしか見えない層:**窓が本当に開かないこと**
      ...
      // ⚠ 2026-09-13(#856 段②): **リンクを足して、リンク先の印を取り込む**ところを
      //    通す 1 本 → 499 → 500。🔴 ここでしか見えない層:**この動線に検査が
      //    1 つも無かった** ── 「リンクを足す」(#401)も「取り込む」も、
      //    smoke を全数 grep して 0 件だった。
      //    🔑 足し先を**このアプリ自身のアドレス**にしたので外へ 1 バイトも出ず、
      //    しかも `index.html` が自分で `<link rel="icon">` を宣言しているため、
      //    **2 段構えの 2 段目(ページを読んで印を取る)が本当に通る**。
      //    ⚠ 既存の道中には載せられない ── URL のタイルを作る smoke が 1 本も無い。
      //    ⚠ 起動を 1 つ増やした(491 → 492/500)
      // ⚠ 2026-09-16(#918 段⑤d-2/d-3): 本文の csv の表が図に出て、押すと SQL が
      //    組まれ、走らせると中身が出るところまでを 1 本で通す → 500 → 501。
      //    ⚠ 既存の道中(`attach.smoke.spec.ts`)には載せられない ── あちらは
      //    **添付の file** を相手にしており、こちらは**本文だけで作った表**なので、
      //    相手そのものが違う(添付を 1 つも作らないことが、この動線の前提である)。
      //    ⚠ 起動を 1 つ増やした(492 → 493/500)
    // ⚠ 2026-09-20(#1017 段⓪): 目次の spec 1 本 → 501 → 502
    // ⚠ 2026-09-21(#1017 段④a): コレクション面の spec(test 1 本、起動 1 回)→ 502 → 503
    // ⚠ 2026-09-21(設計 doc §7、段②a): メッセージの spec 1 本(test 1 件) → 502 → 503
    // ⚠ 段④a と段②a で 502 → 504
    // ⚠ 2026-09-21(#1017 段④b): アーカイブの末尾の spec(test 1 本、起動 1 回)→ 504 → 505
    // ⚠ 2026-09-25(#1042 段⑨): `organize.smoke.spec.ts` に一覧タブの ↑↓ / Enter /
    //    絞り込みの ↓ と、Escape の 2 段決着(面が先・ノートが後)を通す 1 本
    //    → 505 → 506。⚠ 起動は増やしていない ── 面の Escape 決着は同じ台の
    //    末尾に相乗りさせた(smoke-budget が 498/500 で残りが薄いため)。
    // ⚠ 2026-09-26(#1044 段2): `context-menu.smoke.spec.ts` に「見出しの
    //    『この章を編集する』→ 打つ → 別のノートを選ぶと聞かれる → 保存で
    //    履歴が動く」を通す 1 本 → 506 → 507。既存の道中(見出しを畳む test)には
    //    載せられない ── あちらは 1 ノートだけの物語で、こちらは**移る先の
    //    ノートが要る**(物語そのものが違う)。起動を 1 つ増やした。
    // 🔴 2026-09-26(#1044 段2 2巡目の修理、R3): **この pin 自体が実測とずれていた**
    //    ── 同じ commit(4cef044)が「章の欄(390 幅)」describe に test を**もう 1 本**
    //    足していたのに、上のコメントは 1 本しか数えていなかった(実測 508、pin は
    //    507)。507 → 508(数え漏れの訂正)。
    // ⚠ 2026-09-26(#1044 段2 2巡目の修理、R8): 40 行を超える章を開いても
    //    scrollTop が 0(見出しが見える)ことを見る 1 本を足した → 508 → 509。
    //    🔴 ここでしか見えない層:happy-dom は layout を持たないので折り返し・
    //    実際の scroll 位置は測れない ── 本物の版面で「上限に当たったときの
    //    scrollTop」を見るのはここでしか出来ない。既存の道中には載せられない
    //    (道中は 45 行の章を作っておらず、上限に当たらない)。
    // ⚠ 2026-09-26(#1044 段2 2巡目の修理、R9): 見出しに乗せて取っ手を浮かせた
    //    まま章を開き、①取っ手が箱に重なって残らない ②箱の左右端が段落と揃う、
    //    の 2 つを 1 本で通した → 509 → 510。🔴 ここでしか見えない層:本物の
    //    `pointerover` で取っ手が本物の座標に浮くこと・DOM 削除後もそこに
    //    残ること・段落との左右端の揃いは、happy-dom の合成 layout では言えない。
    //    既存の道中には載せられない(道中は取っ手を出す手順を経ていない)。
    // ⚠ 2026-09-26(#1044 段2 3巡目の修理、S2): `context-menu.smoke.spec.ts` に
    //    「45 行の章は見出しもボタンも画面に収まり箱の中でスクロールする / 4 行の
    //    章は内側スクロール無し」1 本(45 行の本命と 4 行の対照群を、起動 1 回の中で
    //    別ノートとして続けて確かめる ── smoke 起動予算(#820)を動かさないため)
    //    → 510 → 511。🔴 ここでしか見えない層:`getBoundingClientRect` /
    //    `elementFromPoint` / `scrollHeight` は happy-dom に無いので、「見出しが
    //    帯に隠れていないか」「ボタンが画面内で押せるか」「箱の中でスクロール
