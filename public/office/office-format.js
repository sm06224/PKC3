 ...
 * ## 測って分かったこと
 *
 * 同じ腕・同じ打鍵で **9 形式**を回した実測(2026-08-23)── **ODF 4/4 が通り、
 * 非 ODF 5/5 が落ちる**(Writer / Calc / Impress のどれでも同じ):
 *
 * | 形式 | Ctrl+S の後 | |
 * |---|---|---|
 ...
 *
 * 🔑 分かれ目は**書式の種類ではなく、ODF かどうか**である。上流の
 * `sfx2/source/doc/objstor.cxx` の `SaveTo_Impl` は、保存先が
 * `IsPackageStorageFormat_Impl`(= 独自の格納形式で、版が 6.0 以上)でないとき
 * だけ `CreateTempFileNoCopy()` の経路を通る ── 配った `writer.xcd` の
 * Word 系フィルタは `FileFormatVersion = 0` なので**必ずそちら**へ落ちる。
 * `.rtf`(zip ですらない)まで落ちるので、Word 系フィルタの話でも zip の話でもない。
 *
