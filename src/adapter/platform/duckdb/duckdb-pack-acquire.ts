 ...
 *
 * 同一オリジンの base から `pack.json`(目録)→ 2 file を**1 つずつ** fetch する。
...
/**
 * 同一オリジンから、目録が指す 2 file を**1 つずつ**取る。
 *
...
export async function fetchDuckDbPackFiles(
  ...
  const names = [DUCKDB_WASM, DUCKDB_WORKER] as const;
...
/**
 * 目録を読んでから 2 file を取る、まとめ口。`duckdb-pack-install.ts` はここだけを呼ぶ。
 *
