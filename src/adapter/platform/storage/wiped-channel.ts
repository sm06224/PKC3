 ...
 *
 * 🔑 **実測で分かった範囲を書く**(着地前レビューが確かめた):捨てた後の worker は
 * `db = null` なので、そのタブが書こうとしても `need()` が投げるだけである ──
 * **幽霊の DB を建てて黙って書き込む経路は無い**。
 * 🔴 それでも残る害は 2 つ:**もう無い一覧を見続ける**ことと、保存しようとして
 * **理由の読めないエラーに当たる**こと(user は捨てたつもりでいるので、
 * 「何が起きたか」を結び付けられない)。
 * ⚠ だから伝える ── 伝えないと、そのタブは**自分だけ時間が止まったまま**になる。
 *
