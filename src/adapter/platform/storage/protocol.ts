...
export type StorageRequest =
  ...
  | {
      ...
      maxRows: number;
      /**
       * 進み具合の見張りが我慢する回数。⚠ **歩数**で切ると、端末の速さで意味が
       *   変わらない(test でも実機でも同じ振る舞いになる)。
       */
      maxSteps: number;
      /**
       * 🔴 **実時間の天井(ms)**。⚠ 歩数だけでは**止まるまでの時間が桁でぶれる**
       *   ── 1 歩の重さが 7 倍以上違う(行を返す形は JS への組み立てが支配的で、
       *   そこは歩数にほとんど数えられない)。実測は `storage-worker.ts` の表に在る。
       * 🔑 別窓(follower)は本体タブへの依頼を **10 秒**で諦めるので、engine が
