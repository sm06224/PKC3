 ...
 * **同じ `assembleDocx`** から出る(`markdown-client.ts` と同じ位置づけ:
 * 「ワーカーは速さの話であって、正しさの話ではない」)。
 */
import { WorkerLease } from '../worker-lease';
import { appJobMonitor } from '../job-monitor';
import { assembleDocx, type DocxJob, type DocxJobResult } from './docx-assemble';

/** アイドルで畳むまで。⚠ 書き出しは連打されないので、短めでよい。 */
export const DOCX_WORKER_IDLE_MS = 15_000;
...
export function setDocxWorkerSpawn(fn: (() => Worker) | null): void {
  lease?.dispose();
  lease = null;
  spawnOverride = fn;
}
...
function leaseOf(): WorkerLease {
  lease ??= new WorkerLease({
    name: 'docx',
    idleMs: DOCX_WORKER_IDLE_MS,
    monitor: appJobMonitor,
    spawn:
      spawnOverride ??
      (() => new Worker(new URL('./docx-worker.ts', import.meta.url), { type: 'module' })),
  });
  return lease;
}

/** ワーカーが生きているか(計測と test の観測点)。 */
export function docxWorkerAlive(): boolean {
 ...
 * を作らない(ワーカーは速さの話である)。
 */
export async function buildDocxFile(req: DocxJob): Promise<DocxJobResult> {
  try {
    return await leaseOf().run<DocxJobResult>(req);
  } catch {
    // ⚠ ワーカーが使えない環境(`file://` / node の unit)も、事故った場合も
    //    **同じ所**へ落ちる ── 返る zip は同じ関数から出る
    return assembleDocx(req);
  }
}
