...
export class InspectorRenderer {
  ...
  render(state: AppState): void {
     ...
     *
     * ⚠ **正直に書き直す**(2026-08-29 の着地前レビュー D-2)。この属性が
     *   「押してから選択が移っても安全」を**担保している証拠は無い** ── この面は
     *   `dispatcher.onState` の**同期** listener で描き直されるので、選択が移った
     *   瞬間にここも書き換わり、**属性と再計算が食い違う窓がそもそも開かない**。
     * 🔑 つまり守っているのは**配線の同期性**であって、この属性ではない。
     *   属性を外す変異は**等価変異**になる(test を足しても殺せない)。
     * ⚠ それでも残す理由は 2 つ:① 非同期の描画へ変えた日に**この属性だけが
     *   正しい値を持つ**(そのときこの docstring が予告になる)② 出す字を
     *   `formatEntryLink` 1 本に寄せる出所として在る。
     */
