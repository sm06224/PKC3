...
export function buildShell(root: HTMLElement): ShellRegions {
  ...
  /**
   * 🔴 **欄そのものが「何を探すか」を名乗る**(#852、user 裁定 2026-09-13)。
   *
   * ⚠ 直す前は `探す` の 2 文字だけで、「**題名と本文から探します**」は
   *   すぐ下の `title`(吹き出し)にしか無かった ── 🔴 **指で触る端末には
   *   吹き出しが出ない**ので、この欄が本文まで探すことは**画面のどこにも
   *   書かれていなかった**。
   * 🔑 user から「**欲しいのは本文を含めたクイックサーチで、いまの左はそうではない**」
   *   と言われて調べたら、**機能はとうに在り、名乗っていないだけ**だった ──
   *   つまり直す所は機能ではなく**字**である。
   * ⚠ アプリの一覧の「探す」タイル(別窓で抜粋と関連度順)とは**別物**なので、
   *   同じ字にしない。
   */
  filter.placeholder = '本文ごと探す';
  /**
  ...
  for (const [value, label] of [
    ...
    ['updated', '更新が新しい順'],
    /**
     * 🔴 **最近開いた順**(#215 残り①)。⚠ **「更新が新しい順」とは別物である** ──
     *   更新は**書いた**ときに動くので、読み返しただけのノートは上がってこない。
     * ⚠ 記録は**この端末だけ**(書き出しにも別の端末にも持っていかない)。
     *   消す口は設定の「最近開いた記録を消す」。
     */
    ['opened', '最近開いた順'],
    ['title', '題名順'],
  ...
  /**
   * 🔴 **前回選んだ種類を、次の起動でも覚える**(C15 / #1045。#1038 段 0 の落差表 N1
   * ── ▼ で選んでも、次に開くとまた「+ ノート」に戻っていた。user の報告ではなく、
   * PKC2 と見比べてこちらが見つけた物である)。
   *
