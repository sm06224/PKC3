 ...
 * 🔴 **予定の札 1 枚**(#292 段③)。
 *
 * ⚠ **2026-09-13 に訂正**:ここには「**2 つの面が同じ札を出す**(板 `kanban.ts` と
 *   予定 `schedule.ts`)」と書いてあったが、**板は #292 段⑤(2026-08-23)で
 *   `VIEW_MODES` ごと外れている** ── 実測で `createTaskCard` を呼ぶのは
 *   `render/schedule.ts` **1 か所だけ**である。
 * 🔑 それでも**組み立てはここ 1 か所に置く** ── 予定の面は**左の列のタブ**と
 *   **別窓**(`ViewMode` の `schedule`)の**両方**が描くので、札の作り方が散ると
 *   「同じ物が場所によって違って見える」が戻ってくる(CLAUDE.md §7)。
 *
 * ## 札が持つもの
 *
 * | 属性 | 誰が読むか |
