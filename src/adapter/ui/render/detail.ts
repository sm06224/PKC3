...
export class DetailRenderer {
  ...
  private renderView(state: AppState, body: string | null): void {
    ...
    if (!lid) {
      ...
      guide.textContent =
        '左の一覧から選ぶと、ここに本文が出ます。まだ何も無いときは、左上の「+ ノート」で作るか、左下の「ファイルを取り込む」で読み込みます。';
    ...
    if (body === null) {
       ...
       * user 報告 2026-09-07「別窓側は再レンダリングでもスクロールは固定して欲しい」)。
       *
       * ⚠ 直す前はここで器を空にしていた ── **本文の高さが潰れるので、ブラウザが
       *   送り位置を 0 に丸める**。本文が戻っても誰も位置を戻さないので、
       *   **読んでいた場所が失われる**。
       * 🔑 実測(2026-09-07、実ブラウザ。別窓で 140 節の本文を開き、主の窓で追記した):
       *
       *   | 観測 | 送り位置 | 本文の高さ | 塊の数 |
       *   |---|---|---|---|
       *   | 追記の前 | **1200** | 10208 | 280 |
       ...
       *
       * ⚠ **位置を控えて戻す形では足りない** ── 戻すのは本文が届いた後なので、
       *   そのあいだ画面が**先頭へ跳ねて戻る**(user から見れば同じ不具合である)。
       *   🔑 だから**そもそも潰さない**:同じノートの本文が既に出ているなら、
       *   新しい本文が届くまで**それを出したままにする**。
       * ⚠ 出るのは**少し古い同じノートの本文**である(別のノートの本文ではない)──
       *   `fresh` でない = `skeletonLid === lid` が成り立っているときだけここを通る。
       * ⚠ 「編集」は押せないまま(すぐ上の `renderBar(state, false)`)── 本文が
       *   届いていないのは事実なので、そこは変えない。
       */
      if (!fresh && this.bodyKind === 'md') return;
  ...
  private renderAttachment(
    ...
    if (meta.assetKey) {
      ...
      if (isAppMime(meta.mime)) {
         ...
         * ⚠ 取り消しは**システムの面**に在る(ここには置かない ── 詳細画面は
 ...
 *
 * ## 🔑 表そのものは共有の 1 本(2026-09-13)
 *
 * ⚠ 段② で**グループの見出しにも**目印を置けるようにしたので、同じ表を出す所が
