...
export function pickCopyFormatInApp(
  ...
  return pickRowInApp(host, {
     ...
     *   **題名と食い違って読めない**。
     * 🔴 **ただし、書き換える行が無い回は名乗らない**(着地前レビュー・動線 D5、
     *   2026-09-06)。⚠ その行は `:::` の囲みの中と**留めた枠**(実装 R1)では
 ...
 * > ようにしたい**」
 *
 * 🔴 **これは「この画面で開く」側である**(#826 で 2 つになった)。
 *
 * ⚠ ここには長く「別窓ではなく、その場の器である ── 選んだら消える一時的な選び手
 *   なので、別窓にすると閉じる操作が 1 つ余計になる」と書いてあったが、
 *   **user に否定された**(2026-09-09):
 *
 * > 「**zipの一覧をその場の器にするのはなんで？/ 別窓にはできないの？/
 * > 普通に別窓で開くとここで開くは共存で、デフォをどちらとするかはユーザー設定では？**」
 *
 * 🔴 実害は「閉じる操作の数」ではなかった ── **`showModal()` なので、開いている間は
 *   本文が 1 文字も見えない**。書庫を開く場面はまさに「**このノートに何が要るかを
 *   確かめながら選ぶ**」ときなので、いちばん見たい物が隠れていた。
 * 🔑 いまは**別の窓が既定**(`platform/archive-window.ts`)で、ここは
 *   ①user が「この画面」を選んだとき ②**別の窓が塞がれたとき**の 2 つで使う。
 *   ⚠ だから**消さない** ── ポップアップを止めている user の唯一の道である。
 *
