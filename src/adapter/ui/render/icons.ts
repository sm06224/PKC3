...
import { symbolChar, type IconName } from '@features/icon/symbols';

export type { IconName };

/**
 * 🔴 **図案の実体は書体になった**(#770 段①、2026-09-11)。
 *
 * > user 要望 2026-09-07:「**アイコン類にマテリアルデザインアイコン(woff2)を採用したい**」
 *
 * ⚠ 直す前は 24×24 の inline SVG を**ここで描いて**いた(40 種・約 170 行の path)。
 *   いまは **Material Symbols の部分集合**(`src/styles/fonts/pkc-symbols.woff2`、
 *   **6,180 バイト**)の 1 文字を置く ── 表は `@features/icon/symbols.ts`。
 *
 * 🔴 **リガチャは使わない。符号位置で置く。**(user の「出ないことがないように」への答え)
 * ⚠ リガチャだと、書体が届く前は「settings」という**英単語がそのまま出る**。
 *   符号位置なら、最悪でも**豆腐(□)**であって、読めてしまう字にはならない。
 *
 * ⚠ **失ったものは `symbols.ts` の表に書いてある**(絵の中の 2 色 / 線の太さの px 固定)。
 */
 ...
 *   変えるとチップが**空になった**(`sidebar.ts` の patch 経路)。
 * 🔑 **書体にしたので、その罠は消えた**(中身は 1 文字なので `textContent` が正しい形である)。
 *   ⚠ 罠が消えたことを書かずに戒めだけ残すと、次に読む人が**在りもしない理由で遠回りする**。
 */
export function setIcon(span: Element, name: IconName): void {
  span.textContent = symbolChar(name);
 ...
 *
 * 🔴 **ボタン丸ごとの `textContent` を読まない**(#770 段①、2026-09-11)──
 *   図案が**書体の 1 文字**になったので、前に**目に見えない 1 文字**が付く。
 *   文言が要るときは `[data-pkc-field="label"]` から採る(製品側は既にそうしている)。
 */
