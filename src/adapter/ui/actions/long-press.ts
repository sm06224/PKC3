 ...
 * | 次の `pointerdown` | 消費窓を**閉じる** | 捨てるのは**同じ押下**の `click` だけ ── 閉じないと、長押しの直後に別の行を素早くタップしたとき、そのタップが黙って消える |
 *
...
export function installLongPress(
  ...
  const onDown = (ev: PointerEvent): void => {
    /**
     * 🔑 **新しい押下が始まったら、消費窓を閉じる**(2026-09-05、実ブラウザの smoke が
     *   教えた)。捨ててよいのは**発火した押下**を離したときの `click` だけで、
     *   それは次の `pointerdown` より**必ず前**に来る。⚠ 閉じないと、長押しの直後
     *   (発火から 700ms 以内)に別の行を素早くタップしたとき、そのタップの `click`
     *   まで飲まれて**押したのに何も起きない**(印が付け替わらない)。
     * ⚠ マウスの判定より**前**に置く ── マウスの押下も新しい押下である。
     */
    fired = null;
    /**
