...
/**
 * 🔴 **SQL の答えを抱える上限**(#681 段② → #918 段③ で 500 から上げた)。
 *
 * ## ⚠ 何を守る数なのかが変わった
 *
 * 🔑 **前は「画面に載せる都合」の数**だった ── 全部 `<tr>` にすると固まるので 500。
 *   ⚠ user 要望 2026-09-14「**取得件数上限もいらない**」はこれに当たっている。
 * 🔑 **いまは描く側が窓になった**(`features/query/sql-window.ts`)ので、
 *   画面の都合は消えた ── 残るのは **抱える費用**(worker の配列 →
 *   `postMessage` の複製 → `AppState`)だけである。
 *
 * ## 🔴 だから「消す」ことはできない
 *
 * ⚠ 止めているのは時間(`SQL_MAX_MS` = 8 秒)と歩数(`SQL_MAX_STEPS` = 20 万)だが、
 *   `storage-worker.ts` に残っている実測が **「再帰 300 万行を返す = 見張り 54,000 回 /
 *   5.6 秒」** と言っている ── **300 万行は 8 秒の内側で返り、歩数の上限にも当たらない**。
 *   消すと、そういう 1 行を打った瞬間にタブが死ぬ。
 *
 * ## 🔑 値の根拠(2026-09-14、窓を入れた版で実測。実アプリ・8 列・各 3 回)
 *
 * | 行数 | `<tbody>` の `<tr>` | 押してから表まで | 引っかかり | 常駐の増分(Pss) |
 * |---|---|---|---|---|
 * | 2,000(窓に入らない) | 2000 | 0.73〜1.37 秒 | **0 本** | +64〜80 MB |
 * | 5,000 | **60** | 1.28〜1.44 秒 | **0 本** | **+19〜26 MB** |
 * | 20,000 | **60** | 1.62〜1.70 秒 | **0 本** | +41〜49 MB |
 * | 50,000 | **60** | 2.18〜2.23 秒 | **0 本** | +62〜81 MB |
 * | 100,000 | **60** | 3.11〜3.39 秒 | **0 本** | +148〜164 MB |
 * | **200,000** | **60** | **4.86〜5.16 秒** | **0 本** | **+237〜266 MB** |
 *
 * 🔑 **`<tr>` が N に依らず 60 本で止まっている**のが、窓が効いている証拠である。
 * 🔑 **20 万行でも引っかかりは 0 本**で、待ちは 5 秒 ── ここから先は
 *   **`SQL_MAX_MS`(8 秒)のほうが先に効く**ので、この数はもう「実質の天井」ではない。
 * ⚠ **20 万より上は測っていない。** 足りなければ測り直して上げる
 *   (⚠ 上げるときは**窓が効いたまま**か = `<tr>` が増えていないかを併せて見る)。
 *
 * 🔴 **測っていない次元がもう 1 つある**(2026-09-14 の着地前レビューが指摘)──
 *   上の実測は**すべて 8 列**である。⚠ `maxRows` / `maxMs` / `maxSteps` の
 *   **どれも列数と 1 升の字数を見ていない**ので、
 *   **多列の結合で 1 升が長い答え**は 1 行あたりの費用がここより高い。
 * 🔑 効いている歯止めは 2 つある(どちらもこの段では触っていない):
 *   ① 1 升は **2000 字**で切る(`storage-worker.ts`)② BLOB は**中身を運ばない**。
 *   ⚠ それでも「8 列で 20 万行が安全」から「多列でも安全」は**導けない** ──
 *   最後に残るのは `SQL_MAX_MS`(8 秒)だけである。
 * ⚠ **測っていないことは主張しない** ── ここが問題になったら、まず
 *   「多列の結合・升いっぱいの字」で同じ 4 つを測り直す。
 * ⚠ 切ったことは `truncated` で言う(黙って切らない)。
 */
export const SQL_MAX_ROWS = 200_000;
          ...
          try {
            ...
            dispatcher.dispatch({
              ...
              message:
                ev.to.toBefore === null
                  /**
                   * 🔴 **ノートの名前は `『』`**(#809-3、2026-09-09。user 推薦 A)。
                   * ⚠ 直す前はここだけ `「」` で、file を落とした回の知らせ
                   *   (`asset-into-note.ts`)は `『』` だった ── **同じ「行き先の
