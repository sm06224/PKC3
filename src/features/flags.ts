...
/**
 * 🔴 **貼り付けたとき、何が届いてどれを使ったかを出す**(user 指示 2026-08-25)。
 *
 * > 「**無言でHTMLペーストを取得する以外のスイッチ経路を用意するなど、
 * > 実用とデバッグを兼用する工夫をしなさい**」
 *
 * ⚠ **切替そのものは設定である**(`pkc3.paste-source` の 4 択)── こちらは
 * **診断**の側:クリップボードに何が載っていたか(形と大きさ)と、なぜその形を
 * 使ったかを 1 行で出す。
 * 🔑 **2 つで 1 組**である ── これが見えるから、user は設定でどれに切り替えれば
 * よいか**分かる**。⚠ 切替だけだと当てずっぽうになり、表示だけだと直せない。
 * ⚠ 出すのは**大きさと判断だけ**(中身は 1 文字も出さない ── 貼ったものは私物で、
 * 画面に出した文字はお知らせの履歴に残る)。
