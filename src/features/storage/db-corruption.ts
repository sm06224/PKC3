 ...
 *   (「拾って書き出す」→「中身を捨てる」→「ファイルを取り込む」)を案内していたが、
...
export const CORRUPT_REFUSAL =
  '保存されている中身の一部が読み込めません。これ以上書き込むと読めない範囲が広がるので、' +
  '書き込みだけ止めました。読むことはできます ── ' +
  `システム の 保存領域 の「保存領域に問題が無いか調べる」で「作り直す・初期化する を出す」を押し、出てきた「${CONTAINER_REBUILD_LABEL}」を押してください。` +
  '読めるノートをファイルへ書き出してから入れ物を作り直し、そのまま戻します(添付は触りません)。' +
  `それでも直らないときは、左下の「${BACKUP_LABEL}」で取り出してから` +
  `「${CONTAINER_RESET_LABEL}」でまっさらにして、左の列の「ファイルを取り込む」で戻してください。` +
