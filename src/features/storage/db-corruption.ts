 ...
 */
import { CONTAINER_RESET_LABEL, RESCUE_ARCHIVE_LABEL } from './rescue-labels';
...
export const CORRUPT_REFUSAL =
  ...
  `設定 の 書き出しと片づけ にある「${RESCUE_ARCHIVE_LABEL}」で読めるノートを取り出し、` +
  `その下の「${CONTAINER_RESET_LABEL}」でまっさらにしてから、左の列の「取り込む」で戻してください。` +
