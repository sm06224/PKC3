 ...
 *
 * かつて `MAX_FENCE_ASSET_BYTES = 2MB` を置き、超える添付は**読まずに理由を出して**
 * いた(#444 段①)。理由は「配る量ではなく**定常**の話 ── 大きい字を毎回運ぶと
 * 開くたびに払う」だった。
 * 🔑 ⚠ その心配自体は消えていないが、**user が置いた物を開けないほうが害が大きい**。
 *   重さの対処は**上限で断ること**ではなく、**読み方を軽くすること**である
 *   (ワーカーへ逃がす / 遅延で読む ── #492 段②)。
 */
