 ...
 *
 * ⚠ **fence(``` の中)は markdown-it が先に tokenize する**ので、inline rule には
 * 届かない ── だから**前処理で置換してはいけない**(置換すると、その守りを自分で捨てる)。
 * 🔴 **ただし code span(`` `…` ``)は、それだけでは守れない**
 *   (着地前レビュー 2026-09-06)── 開きの `$` が code の**外**に在ると、
 *   走査が中の `$` を閉じに使う。だから下の走査は**逆引用符で止まる**。
 */
