 ...
 *
 * 🔴 **入れ子の `:::` は今日の描画が壊れている**(2026-08-05 実測。本 stage とは
 * 無関係な既存バグ):`:::section` の中の `:::note` がリテラルの段落になり、
 * 外側の閉じ `:::` が `<p>:::</p>` として漏れる。ここでは**原文として正しい範囲**を
 * 返す ── 描画と食い違うので `buildBlockPartition` の検証が落ち、
 * 行の差し替えを**開かない**(= 今日の編集画面へ退避する)。壊れた分割の上で
 * 編集させるより安全側である。
 *
