 ...
 * 見出しに id を焼き、目次を組む。
 *
 * ⚠ **既に `id` を持つ見出しも上書きする** ── h1〜h3 の slug をそのまま使うと、
 *   h4〜h6 だけ別の付け方になり、**目次の中で 2 つの規則が混ざる**
 *   (CLAUDE.md §7「判定を増やさない」)。番号 1 本に寄せる。
...
export function buildManualDoc(
  ...
  const html = stripped.replace(HEADING_OPEN, (_m, level: string, attrs: string | undefined) => {
    ...
    const tag = `<h${level} id="${MANUAL_HEADING_ID}${n}"${rest}>`;
    n += 1;
    return tag;
  });
  ...
  for (const s of sections) {
    ...
    toc.push({
      targetId: `${MANUAL_HEADING_ID}${s.index}`,
      label: label(s.title),
      level: s.level,
    });
  }
