 ...
 * ⚠ 現行の `generateLid()` は **`<epoch36>-<counter36 4 桁>-<乱数 8 桁>`**
 *(2026-09-16 に 3 段目を足した ── #973)なので前置の衝突は
 * まず起きないが、🔑 **ここの効き方は段の数に依らない**
 *(見ているのは「次の 1 文字が lid の文字でないこと」である)。
 * ⚠ そして**取り込んだ容れ物の lid は形が違う**(PKC2 由来)。
 * 「たぶん起きない」を検査の根拠にしない。
 */
