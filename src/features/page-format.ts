 ...
 *    使わない」に戻る)
 * ② 図のラスタ幅は器の親の `clientWidth` から決まるので、器に上限を掛けると
 *    **どの紙面でも全図が焼き直される**。
 *
 * ⚠ **②は 2026-09-06(#722 P2-11)に半分だけ崩れた** ── user 裁定で
 *   「表・図・コードも段落と同じ左端に揃える」ことにしたので、塊に
 *   `margin-inline-start` が付き、**紙面を変えると図は焼き直される**
 *   (実測: 焼き幅 A4 848px ↔ フル HD 1040px)。器そのものは今も動かないので①は
 *   生きている。⚠ 「図は 1 枚も動かない」と書いてあった 4 か所を同時に直した
 *   ── **直せない主張を残すと、次に読む人が誤った前提で判断する**。
 *
