 ...
 *
 * 🔑 **左の列に同じ面が在るものは、そのタブと同じ絵にする**
 *   (`render/icons.ts` の `BROWSE_ICONS`):予定表 = `calendar` / 連絡先 = `person` /
 *   音・動画 = `mic`。⚠ 同じ面の 2 つ目の入口なので、絵が違うと**別物に見える**
 *   (2026-09-04 の裁定「入口を 2 つにする(片方を消さない)」の見た目版)。
 * ⚠ 逆に **`folder` は使わない** ── 左の「フォルダ」タブが既にその絵なので、
 ...
 * ⚠ 並びは**マニュアルの次**── これは「作業する所」でも「読む所」でもなく、
 *   **1 度だけ使う支度**である。ここに置けば、足しても他のタイルの位置が 1 つも動かない。
 * ⚠ **「いちばん最後」ではない**(2026-09-12 に訂正)── 足したときは末尾だったが、
 *   その後 `sql`(#681 段②)と `captures`(#683 段①)が**同じ理屈で後ろへ**付いた。
 *   🔑 「いま何番目か」を注記に書くと、次に 1 枚足した日に嘘になる ── **書くなら
 *   「なぜそこか」だけにする**(この repo が繰り返し踏んでいる型)。
 */
...
export function withBuiltinTiles(
   ...
   * ⚠ **組み込みの中の順番は変えない** ── Office の有無で位置が動かない向きはそのまま。
   * ⚠ **ここに順番を書き写さない**(2026-09-12 に訂正)── 直す前はここに
   *   「2 ペイン → 予定表 → 連絡先 → 探す → Office → マニュアル」と**全順の顔で**
   *   書いてあったが、`selfhost` / `sql` / `captures` の 3 枚が抜けていた。
   *   🔑 **並びの正本はすぐ下の `builtin.push(...)` の順**であり、
   *   マニュアル側との突合は `tests/docs-parity.test.ts` が見る。
   */
