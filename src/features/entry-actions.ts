...
/**
 * 🔴 **アプリのタイルを右クリックしたときに出す物**(#857 段①)。
 *
 * ## なぜ「掴んで落とす」だけにしないのか
 *
 * ⚠ 掴んで落とすは**掴める端末・掴める人**にしか効かない ── 指だけの端末、
 *   手が震える人、トラックパッドの押しっぱなしが苦手な人には道が 1 本も無くなる。
 * 🔑 フォルダの面と**同じ作法**(あちらも帯に「上へ / 下へ」を持っている)。
 *
 * ⚠ **2 つだけにする** ── 「先頭へ」「末尾へ」は押し所を増やすわりに、
 *   掴んで落とすで足りる(増やすなら理由を 1 つずつ書く、という #426 の規律)。
 * ⚠ 出すのは**動かせるタイル**の上だけ(組み込みには出さない)── 判定は
 *   `isMovableTile` 1 本で、ここには条件を書かない(`when` は entry の種類を見る
...
export const APP_GROUP_RESET_ACTION: EntryAction = {
  ...
  /**
   * 🔴 **効く範囲を字に出す**(2026-09-13、着地前の動線レビュー)。
   *
   * ⚠ 直す前は「**名前順に戻す**」だった ── 同じメニューの「上へ / 下へ」は
   *   **押した見出しだけ**に効くので、user はこれも「この群だけ」と読む。
   * 🔴 **実際は番号を持つ群を全部戻す** ── 「資料」の見出しから押したのに、
   *   見てもいない「作業」の並びが消える(押した物と効く先が食い違う。#677 の型)。
   * 🔑 群ごとに戻す形にはしない ── 1 群だけ番号を外すと**その群だけ末尾へ飛ぶ**
   *   (番号のある群が先に来る規則なので)── user の求めは「元の並びへ」であって
   *   「この群を末尾へ」ではない。⚠ だから**全部戻すのが正しい**が、
   *   **そう書いていなかった**のが欠陥である。
   */
  label: 'すべて名前順に戻す',
};
 ...
 * ## なぜ「並べ替える」が要るのか
 *
 * ⚠ 長押し(指・ペン)で並べ替えモードに入れるようにしたが、**マウスは長押しを
...
export function bodyMenuActions(
   ...
   *   `adopt-external-images` は**本文の右クリックにしか無い**(マニュアル「画面を組み替える」 が
 ...
 */
/**
 * 🔴 **形の項目**(#530 案 A)。⚠ **綴りを字で書く** ── `` `place-shape-${sh}` `` と
 *   組み立てると、**出口の全数走査**(`scripts/action-outlets.mjs` は
 *   `action: '…'` の字を読む)から見えなくなり、「静的に追えない出口をまた足した」
 *   として `tests/action-outlets.test.ts` が落ちる(受け手側も同じ理由で字で書いてある)。
 * 🔑 呼び名は `PLACE_SHAPE_LABELS` の 1 本から採る(§7)。
 * 🔑 `Record<PlaceShape, …>` なので、**形を足して書き忘れたら tsc が落ちる**。
 */
const SHAPE_ACTION: Record<PlaceShape, EntryAction> = {
  rect: { action: 'place-shape-rect', label: `${PLACE_SHAPE_LABELS.rect}にする` },
  round: { action: 'place-shape-round', label: `${PLACE_SHAPE_LABELS.round}にする` },
  ellipse: { action: 'place-shape-ellipse', label: `${PLACE_SHAPE_LABELS.ellipse}にする` },
  diamond: { action: 'place-shape-diamond', label: `${PLACE_SHAPE_LABELS.diamond}にする` },
  arrow: { action: 'place-shape-arrow', label: `${PLACE_SHAPE_LABELS.arrow}にする` },
};
 ...
 *   「CSV の表にする」で読めるが、こちらは**コピーの形が 5 つ並んだ下**に出るので、
 *   同じ字だと「**CSV でコピーする**」と読める ── 押すと本文が書き換わる。
 * 🔑 だから「**本文を**」を頭に置き、「**書き換える**」で締める。
 * ⚠ 向き(どちらの形にするか)は {@link tableMenuActions} と**同じ引数**から出す ──
 ...
 *   `Alt`+クリックで同じことができる(マニュアル「見出しを右クリックする」)のに、メニューからは知りようがない。
 ...
 * 🔑 右クリックは「右の列を畳んだ人のための 2 本目の道」(マニュアル「画面を組み替える」)なので、
 *   **説明が要るのはむしろこちら**である。
 *
...
export const ENTRY_ACTION_HINTS: Readonly<Record<string, string>> = {
  ...
  'export-entry': 'このノートの本文と添付を保存します(.pkc3-notes.zip)。取り込み直せます',
