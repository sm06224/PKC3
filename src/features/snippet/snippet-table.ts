...
export const SNIPPET_LIMITS = {
   ...
   *
   * 🔑 **測った**(2026-08-25。主張は「主スレッドが抱える retained heap」であって、
   *   プロセス木の Pss ではない ── worker の常駐はこの計器では語れない):
   *   | 量 | 抱える |
   *   |---|---|
   *   | **上限いっぱい**(200 件 × 4000 字) | **1.56MB**(2.0 バイト/字 = UTF-16 のまま) |
   *   | 多め(80 件 × 800 字) | 0.13MB |
   *   | 現実的(30 件 × 300 字) | **0.02MB** |
   *   ⚠ 対照群(同じものを作って捨てる)は 0.00MB ── 抱えている分だけが出ている。
   *   ⚠ 1 稿目は `slice` で作った字を測って **9.88MB**(12 バイト/字)と出た ──
   *     V8 が親の字を抱えたままだったからで、実物は `postMessage` の clone を
   *     通って平たくなる。**計器のほうが間違っていた**(CLAUDE.md §4)。
   */
  notes: 200,
