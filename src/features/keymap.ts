...
export interface KeyCommand {
   ...
   * 既定の割当(複数可 = 別名)。
   *
   * ## ⚠ 「空にしない」は**無条件ではない**(2026-09-13 に読み直した。#582 段②-2)
   *
   * 🔑 この戒めの**目的**は「**user が割当を変えたとき、元へ戻せなくなる**」ことを
   *   防ぐことである(`isDefault` / 「既定に戻す」が指す先が無くなる)。
   * ⚠ だから**一度も鍵を持たない操作には、この理由が当たらない** ── 戻る先が
   *   「割当なし」なので、戻し道は失われない(設定画面は `list.length === 0` を
   *   既に「**割当なし**」として描く)。
   *
   * 🔴 **空にしてよいのは「名前で呼ぶだけの操作」である。**
   *   パレット(`palette-rows.ts`)はこの表を回して行を作るので、**鍵を要求すると
   *   「名前で呼べる」= 「鍵が割り当ててある」になり、鍵を付けられない操作
   *   (実測 151 件)が 1 つもパレットに出せない**。
   * ⚠ 空にした id は `tests/features/keymap.test.ts` の `KEYLESS` に
   *   **身元で**足すこと ── 数で縛ると、うっかり空にした物と区別が付かない。
   */
...
export const KEY_COMMANDS: readonly KeyCommand[] = [
   ...
   *
   * ⚠ 入口が**本文に打つことだけ**だった ── 「操作を探す」で「計算」と打っても
   *   **0 行**、書式パネルにも無い。マニュアル自身が「名前を忘れても**操作を探す**で
   *   引けます」と書いているのに、この機能だけそこに居なかった。
   * 🔑 **画面に物は 1 つも増えない**(探したときだけ出る)── 書式の帯は 14 個で
   *   横に長く、これ以上ボタンを増やせない(#766 D-3 を退けた理由)。
   * ⚠ 既定の鍵は**空にできない**(この表の規則)── `Alt+Shift+C`(Calc)は
   *   他のどの既定とも重ならず、ブラウザも取らない。
   * ⚠ `note` に**打ち方**を書く ── 一覧は `label` と `note` の両方を見るので、
   *   「けいさん」でも「2+3=」でも当たる。
   */
  {
    id: 'inline-calc',
  ...
  /**
   * 🔴 **文字を打っている間にも面へ移れる別名を足す**(#721。cowork 評価レポート)。
   *
   * ⚠ mac で `⌥+1` / `⌥+2` / `⌥+6` が「打っている最中に効かない」と報告された。
   *   実装を読むと**壊れてはいない** ── `Alt+<数字>` は `typesCharacter` が真
   *   (mac では `⌥1` が `¡` を打つ)なので、打鍵中の門が**わざと**止めている。
   * 🔑 だから直すのは門ではなく**入口の数**である:`Mod` を足した和音は
   *   `typesCharacter` が偽になるので、同じ操作へ**打鍵中でも通る道**が 1 本増える。
   * ⚠ **既定は外さない** ── 体で覚えた `Alt+1` はそのまま効く(#241 段⑥-a と同じ
   *   理由。保存済みの割当もずらさない)。`open-settings` の
   *   `['Alt+3', 'Mod+Comma']` が先にこの形を取っている。
   * ⚠ **別名を足すだけでは効かない** ── 門は「コマンドが名乗る **かつ** その和音が
   *   文字を打たない」の 2 条件なので、`whileTyping` を併せて立てる。立てないと
   *   `note` が嘘になる(CLAUDE.md「『Alt+3 で開けます』と書いてはいけない」)。
   * 🔑 本文の面は**編集の面そのもの**なので、戻るのを止める理由が無い
   *   (`SET_VIEW_MODE` の「本文へ戻る道は塞がない」と同じ判断)。集計は編集中は
   *   引き続き**声に出して断る** ── 打鍵中でも「絞り込み欄に打っている」ような
   *   編集中でない場面では、そのまま開く。
   */
  {
    id: 'view-detail',
   ...
   *
   * ⚠ 直す前は「操作を探す」で「窓」「付箋」「ウィンドウ」と打っても **0 行**だった ──
   ...
   */
  {
     ...
     * 🔴 **2 つ目の既定を足した**(#721)── ここには長く「こちらで勝手に別の鍵を
     *   足さない(既に体で覚えた割当をずらさない)」と書いてあったが、⚠ その戒めが
     *   守っているのは**既に在る割当をずらさないこと**であって、**別名を増やさない
     *   こと**ではない(CLAUDE.md「戒めには何のための禁止かを書く」── 目的に照らすと
     *   `Alt+6` を残したまま足すのは禁止の対象外)。user は打鍵中に開けずに困っていた。
     */
   ...
   *
   * ⚠ この面は aside なので、**左の一覧でノートを 1 回押すと中央が本文へ戻る**
   *   (それ自体は正しい)。⚠ ところが SQL には**左のタブが無く**、鍵も
   *   操作パレットの行も無かったので、**戻る道が画面に 1 つも無かった**
   *   ── アドレスに手で `#pkc?view=sql` と打つ以外に戻れない。
   * 🔑 だから `view-dual` と同じ形で 1 行足す(**番号は末尾に足す** ── 既に
   *   `Alt+1`〜`Alt+6` を体で覚えた user と、保存済みの割当をずらさない)。
   * ⚠ これで**打ちかけの字が残っていることを user が確かめられる**ようになる
   *   (マニュアルはそう約束していたのに、確かめる道が無かった)。
   */
  {
    id: 'view-sql',
    label: 'SQL へ',
 ...
 * ⚠ export しているのは **マニュアル「ショートカットキー」 の一覧と突き合わせる**ため(#697、
