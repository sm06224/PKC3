...
const ACCEPTED_ARCHETYPES: ReadonlySet<string> = new Set([
  ...
  'stack',
  // 🔴 **スマートフォルダも受ける**(#870、2026-09-13)── 雛形・スタックと**同じ理由**で
  //    入れるべきだったのに、**入れ忘れていた**(この 2 つの理由がすぐ上に書いてあるのに)。
  //    ⚠ 条件は frontmatter の平らな key なので、単一 md に収まる(`smart-spec.ts`)。
  //    受けないと、書き出したスマートフォルダを取り込み直したとき**普通のノートに化け**、
  //    **条件で集める働きだけが静かに消える**(本文は残るので壊れて見えない)
  'smart',
  // 🔴 **グループ用のノートも受ける**(#857 段②)── 受けないと、書き出した
  //    グループを取り込み直したとき普通のノートに化け、**見出しの目印だけが消える**
  //    (本文は残るので壊れて見えない)。中身は frontmatter 1 行で単一 md に収まる
  'appgroup',
  'folder',
  'generic',
  'opaque',
