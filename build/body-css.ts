 ...
 * 全 320 規則が「`.pkc-md-rendered` を起点にする 123 本」と「それ以外 197 本」に
 * 機械的に割れる。
 * ⚠ **かつて境界例だった読み幅の 1 本は、いまは本文側に居る**(user 裁定 2026-08-08)──
 *   `.pkc-md-rendered[data-pkc-field='detail-body'] > :is(…)` 起点にしてあり、
 *   **意図して焼いている**。器に上限を置いているのではなく、器は絞り込みであって
 *   上限は各ブロックに掛かる。⚠ 起点を器側(`[data-pkc-view-pane=…]` など)へ戻すと
 *   **配った HTML から読み幅が消える** ── 旧記述の「落ちて正しい」を復活させないこと。
