 ...
 *
 * 🔴 **2026-09-06(#707)に 7500 → 8300 KB へ引き上げた**。数式(KaTeX)を入れたため。
 * 内訳は実測で **CSS 23.3 KB + 書体 woff2 19 本 250.2 KB**(JS は worker の塊なので
 * 数式を書くまで読み込まれない)。user 裁定 2026-09-06「入れる」。
 *
 * 🔴 **⚠ ここまでの版はこの行に「mermaid / katex 級の誤取込は今も止まる」と
 * 書いていた** ── katex を**止めるべき事故の例**として名指ししていたので、
 * そのままにすると**次に読む人が「止めたはずの物が入っている」と読む**。
 * 🔑 いま katex は**裁定済みの実装**である。誤取込の例として引くなら別の名前にする。
 *
 * 🔴 **そして「引き上げて通す」の前に、取り込みを直した** ── 素のまま入れると
 * KaTeX の `@font-face` が **woff2 / woff / ttf の 3 形式 59 file** を出し、
 * **読まれない woff と ttf が 797.6 KB**(実測)precache に載っていた。
 * `build/katex-woff2-plugin.ts` で woff2 だけに絞ってから引き上げている ──
 * ⚠ **cap を上げるのが先になると、誤取込がそのまま固定される**。
 * 🔑 余裕は約 500 KB ── **誤取込 1 本(数百 KB〜MB)は今も止まる**。
 *
 * 🔴 **2026-09-13(#856 段②)に 8300 → 8800 KB へ引き上げた** ── ⚠ 触ったからでは
 * なく、**余裕が無くなって tripwire でなくなっていた**からである。
 * 実測:段② が足したのは **+9.8 KB**(8265.7 → 8275.5)だが、それで残量は
 * **24.5 KB(0.3%)** になった ── この幅では**通常の実装が毎回ここで止まる**。
 * 🔴 それは user 指示 2026-08-28「**予算はあくまで何かの手違いを検出するために
 * 設定してる**」と逆で、**サイズを守らせる規律**になってしまっている。
 * 🔑 だから「約 500 KB の余裕」という、この file が繰り返し書いてきた形へ戻す
 *   ── **誤取込 1 本(数百 KB〜MB)は今も止まる**。⚠ **撤廃はしない**。
 *
 * 🔴 **2026-09-20(#1007 段①)に 8800 → 9300 KB へ引き上げた** ── 同じ理由である。
 * 実測:段① が足したのは **+2.2 KB**(8793.7 → 8795.9)だが、残量は
 * **4.1 KB(0.0%)** で、次の通常増加が必ずここで止まる幅だった。
 * 🔑 「約 500 KB の余裕」へ戻す ── **誤取込 1 本は今も止まる**。⚠ **撤廃はしない**。
 */
const SHIPPED_CAP_KB = 9300;
 ...
 * 🔑 実測 **41375.4 KB**(器は 2026-09-15 / 拡張は 2026-09-16):
 *   `duckdb-eh.wasm` **35072.0 KB** + `duckdb-browser-eh.worker.js` **755.1 KB**
 *   + 拡張 3 つ **5548.3 KB**(parquet 3142.9 / sqlite_scanner 1603.2 / json 802.2)。
 *   ⚠ 素の版(`eh`)だけ ── スレッド版(`coi`)は実測で遅く・重く・壊れやすかった
 *   (設計 doc §5)ので配らない。
 * ⚠ 余裕は約 4620 KB ── 上流の版上げと拡張の育ちは吸うが、**もう 1 つの版を
 *   誤って取り込む**(`mvp` 39.4 MiB / `coi` 34.0 MiB のどちらか)は止まる。
 * ⚠ **配る量は判断理由にしない**(不可侵指示 2026-08-03)── ここは手違いの検出である。
 * ⚠ 下限は「空 / 途中で切れた一式」だけを狙う(実測の半分弱)。
 *   🔴 **拡張が 1 つ落ちる**のは量では止まらない(総量の 2%)ので、
 *   そちらは `dist-inspect.mjs` の `DUCKDB_REQUIRED` が**集合で**見る。
 */
const DUCKDB_CAP_KB = 46000;
