 ...
 *
 * 入力は workflow の `toJSON(steps)` を素通しした `PKC3_NIGHTLY_STEPS`:
 *
...
/**
 * `toJSON(steps)` から **揃わなかった step**を拾う(`id:outcome` の並び)。
 *
...
export function unmetSteps(stepsJson) {
  ...
  if (!steps || typeof steps !== 'object' || Array.isArray(steps) || Object.keys(steps).length === 0)
    // 🔴 空を「緑」と読まない ── `steps` に入るのは **id を宣言した step だけ**なので、
    //    id を落とすと静かに空になる。空は集計の故障であって、良い知らせではない
    throw new Error('STEPS が空 ── step に id が無いか、渡し忘れている');
  const bad = [];
  for (const [id, v] of Object.entries(steps)) {
    const outcome = typeof v?.outcome === 'string' ? v.outcome : '';
...
export async function reconcileNightlyIssue({ fetch, repo, token, failedSteps, runUrl, today }) {
  ...
  const created = await call('POST', `/repos/${repo}/issues`, {
    ...
    body:
      ...
      `(戻ったら自動で閉じます)。\n\n揃わなかった step(落ちた / 走らなかった):\n${steps}\n\n${runUrl}\n\n` +
      `⚠ 夜の検査は **1 つ落ちても後続を止めません**(#221)。並んでいる step が複数なら、` +
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  console.log(
    failedSteps.length > 0 ? `🔴 揃わなかった step: ${failedSteps.join(', ')}` : '✅ 全部緑',
  );
