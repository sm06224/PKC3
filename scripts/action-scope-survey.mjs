...
open-repeat-menu set-task-repeat toggle-app-group pick-app-group-icon
move-app-group-up move-app-group-down`.split(/\s+/);
...
rename-entry-begin move-to-folder create-in-folder swap-open adopt-link-icon`
  .split(/\s+/);
...
rename-attachment toggle-app-tile set-app-group set-app-icon set-sql-text set-sql-source`.split(/\s+/);
